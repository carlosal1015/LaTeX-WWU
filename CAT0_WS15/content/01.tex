...
\begin{beispiel}
\label{bsp:1.6}
	Geodätische Dreiecke sind im Allgemeinen durch ihre Ecken nicht eindeutig bestimmt.
	\begin{figure}[h]
		\centering
		...
		\draw[->,very thick, color=SeaGreen4] (1,0.02) -- (-1,0.02);
		\end{tikzpicture}
		\caption{Zwei verschiedene geodätische Dreiecke mit den gleichen Eckpunkten.}
	\end{figure}
\end{beispiel}
...
\begin{beweis}
	...
	\begin{align*}
		...
		\stack{\eqref{eq:1.19.3},\eqref{eq:1.19.4}}{\leq} &\frac{\varepsilon}{2} + \frac{\varepsilon}{2} = \varepsilon \quad \forall n,m \geq \max(N_1,N_2). 
	\end{align*}
\end{beweis}
...
\label{lemma:1.26}
	...
	Wenn $\Fix_\Phi(G) \neq \emptyset$, dann ist $\Fix_\Phi(G)$ konvex. (Aufgabe~\ref{aufg:2.2}) \index{konvex}
\end{lemma}
...
\label{kor:1.30}
	...
	\begin{enumerate}[(i)]
		...
		\]
		Dann enthält $G$ nur endlich viele verschiedene Konjugationsklassen von endlichen Untergruppen.
		Genauer: Definiert man auf der Menge $\UGend := \{U \leq G \text{ endlich}\}$ die Äquivalenzrelation \index{Konjugationsklasse}
		\[
...
\begin{lemma}
\label{lemma:1.35}
	Ein vollständiger metrischer Raum, der Mittelpunkte hat, ist geodätisch. (Aufgabe~\ref{aufg:1.3})
\end{lemma}
...
\label{lemma:1.37}
	...
	$d(x,\beta(s)) \leq d_2(\ol{x},\ol{\beta}(s))$ für alle $s \in [0,d(y,z)]$. (Aufgabe~\ref{aufg:3.2})
\end{lemma}
...
\begin{beweis}
	...
	Nach Lemma~\ref{lemma:1.35} reicht es zu zeigen, dass $X$ Mittelpunkte hat.
	...
	Wir betrachten die Bedingung aus Lemma~\ref{lemma:1.37}. Sei $p := \beta(s) \in \im(\beta \colon y \geo z)$ beliebig.
	...
	\]
	Folglich liegt $\ol{p}$ auf der Geodäte $\ol{\beta}$, das heißt wir haben ein Vergleichsdreieck. 
	\end{description}
\end{beweis}
...
\begin{beweis}
	...
	Wir zeigen, dass die Bedingung aus Theorem~\ref{thm:1.38} erfüllt sind.
