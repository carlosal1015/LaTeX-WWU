...
\begin{definition}[{name=[Funktionskeime]}]
	...
	Ein \Index{Funktionskeim} auf $M$ um $p$ ist ein Paar $(U,f)$, wobei $U \subseteq M$ eine offene Umgebung von $p$ und $f \colon U \to \mathbb{R}$ eine $C^\infty$-Abbildung ist.
	
	Zwei Funktionskeime $(U,f)$ und $(U',f')$ sind \bet{äquivalent}\index{Funktionskeim!äquivalent}, falls es eine Umgebung $p \in U'' \subseteq U \cap U'$, sodass $f|_{U''} = f'|_{U''}$ gilt.
...
\begin{definition}[{name=[Tangentialvektor und Tangentialraum]}]
	Sei $M$ eine Mannigfaltigkeit, $p \in M$.
	Ein \Index{Tangentialvektor} an $M$ in $p$ ist eine lineare Abbildung $X \colon C^\infty_p(M) \to \mathbb{R}$, sodass 
	\[
		X(fg) = X(f) \cdot g(p) + f(p) \cdot X(g)
	\] 
	gilt.
	Der \Index{Tangentialraum} $\Tang_p M$ ist die Menge aller Tangentialvektoren an $M$ in $p$.
...
\begin{beispiel}[{name=[offene Teilmenge des euklidischen Raumes]}]
	...
	\mapdef{\eta_p \colon \mathbb{R}^n}{\Tang_p U}{v}{\eta_p(v)}{}
\end{beispiel}
...
\begin{lemma}[label=lem:summe-fkt]
	...
	Dann existieren $C^\infty$-Funktionen $f_1, \ldots ,f_n \colon U_0 \to \mathbb{R}$, sodass\marginnote{$f_j(p)= \diff{f}{x_j}(p)$}
	\[
...
\begin{beweis}[name={von \cref{lem:eta-iso}}]
...
\mapdef{f^* \colon C^\infty_{f(p)}(N)}{C^\infty_p(M)}{g}{g \circ f}{}
...
Eine weitere Folgerung:\todo{Nummer?}
Sei $f \colon M\to N$ differenzierbar und $p \in M$.
