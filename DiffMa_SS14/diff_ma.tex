...
Startpunkte $\widehat{=}\,{\tikz[baseline=-0.5ex]{\draw[fill, RoyalBlue2] (0,0) circle[radius=0.04];}}$ und $t \in [-1, 1]$\tikzexternaldisable 
\[
...
\end{psmallmatrix*}$ ergibt sich $e^{A \cdot t} = \begin{psmallmatrix*}[l]
	...
	e^{-t} & 1
\end{psmallmatrix*}$.
Startpunkte $\widehat{=}\,{\tikz[baseline=-0.5ex]{\draw[fill, RoyalBlue2] (0,0) circle[radius=0.04];}}$ und $t \in [-1/2, 1/2]$\tikzexternaldisable 
\[
	\begin{tikzpicture}[scale=1.2]
		%\draw[help lines] (-3,-3) grid (3,3);
...
\enquote{Schwingen} einer Ebene um den Nordpol des $S^2$
