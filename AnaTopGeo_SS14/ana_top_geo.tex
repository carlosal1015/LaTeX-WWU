...
\label{sub:131}
Sei $p \colon \hat{X} \to X$ eine Überlagerung. Sei $x_0 \in X, \hat{x}_0 \in \hat{X}$, $p(\hat{x}_0)= x_0$. Sei $Z$ wegzusammenhängend und lokal wegzusammenhängend. 
...
\]
\minisec{Beweis}
Existiert $\hat{f}$, so folgt $(\star)$ aus $f_* = p_* \circ\hat{f}_*$. 
Sei umgekehrt $(\star)$ erfüllt. 
Sei $z\in Z$. Sei $\omega \colon [0,1]\to Z$ ein Weg von $z_0$ nach $z$.
Sei $\hat{\omega} \colon [0,1] \to \hat{X}$ die eindeutige Hebung von $f \circ \omega$ mit $\hat{\omega}(0)= \hat{x}_0$ (Homotopiehebungssatz, \ref{sub:108}). 
...
\begin{description}
	...
	Sei $\hat{\eta} \colon [0,1] \to \hat{X}$ die zugehörige Hebung. Zu zeigen: 
	$\hat{\eta}(1) = \hat{\omega}(1)$. 
	...
	Aus $(\star)$ folgt, dass $\benbrace*{f \circ (\overline{\omega} * \eta )}$ im Bild von $p_* \colon \pi_1(\hat{X}, \hat{x}_0) \to \pi_1(X,x_0)$ liegt. 
	...
	es eine wegzusammenhängende Umgebung $W$ von $z$ mit $f(W) \subseteq V$. Sei nun $\omega \colon [0,1] \to Z$ ein Weg von $z_0$ nach $z$.
