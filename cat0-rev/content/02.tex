\section{Der Satz von \textsc{Cartan-Hadamard}} 
\label{cha:2}
...
\begin{minipage}{0.6\textwidth}
	\begin{no-erinner}[{siehe \cite[Kap. 3]{Kramer}}]
...
\end{align*}
Das ist eine Überlagerung, denn für $b = (\cos(\alpha),\sin(\alpha)) \in \Ss^1$ und $U_b = \Ss^{-1} \setminus \{-b\}$ ist $p^{-1}(U_b) =$ \linebreak $\bigcup\penbrace*{ \enbrace*{k+r-\frac{1}{2},k+r+\frac{1}{2}} : k \in \ZZ}$.
...
\begin{beweis}[Korollar~\ref{kor:2.2}]
	\begin{description}
...
\begin{beweis}[Theorem~\ref{thm:2.13}(ii)]
	Seien $x,y \in X, x\neq y$ beliebig.
...
\label{bem:2.22}
	...
	\begin{figure}[h]
		...
		\end{tikzpicture}	
		\caption{Veranschaulichung der Konstruktion aus Bemerkung~\ref{bem:2.22}.}	
	\end{figure}	
\end{bemerkung}

\subsection{Längenmetrik auf Überlagerungsräumen}
\label{sec:2.2}
...
\begin{beweis}
	...
	\textit{Zur Eindeutigkeit:} Aufgabe~\ref{aufg:5.2}.
...
\begin{beweis}[Fortsetzung von Satz~\ref{satz:2.31}]
	\begin{enumerate}[(i)]
		\setcounter{enumi}{1}
		\item Sei $c \in \tilde{X}_{x_0}$ beliebig und $\varepsilon > 0$ so, wie in Lemma~\ref{lemma:2.32}.
		...
		\begin{itemize}
			...
			Nach Lemma~\ref{lemma:2.32} existiert eine linear umparametrisierte Geodäte $c_{x_0 \ol{y}}\colon [0,1] \rightarrow X, x_0 \geo \ol{y}$ mit $d(c_{x_0 \ol{y}},c) = \sup\limits_{t \in [0,1]} d_X(c_{x_0 \ol{y}}(t),c(t)) < \varepsilon$.
...
\vspace*{1em}
	...
	\begin{minipage}{0.65\textwidth}
		...
		Sei weiter $\varepsilon > 0$ so, wie in Lemma~\ref{lemma:2.32}.
...
\label{prop:2.43}
	...
	Dann gilt (Aufgabe~\ref{aufg:6.4}):
	\[
...
\begin{beweis}
	\begin{description}
		...
		\]
		Dann folgt mit Lemma~\ref{lemma:2.45}, dass $X$ $\CAT$ ist.
		...
		Mit Lemma~\ref{lemma:2.44} folgt:
		\[
			d(x,m) = d_2(\ol{x},\ol{\ol{m}}) \leq d_2(\ol{x},\ol{m}). 
		\]
	\end{description}
\end{beweis}
...
\begin{beweis}
	...
	\item[2. Fall:] $k=1, l > 1$.
		...
		Anschließend ist Fall 1 benutzbar. 
	\end{description}
\end{beweis}
...
\label{bew:tech_lemma}
	...
	\begin{enumerate}[(1)]
		...
		Die Komposition $\alpha\big|_{[a,a_2]} * \beta \big|_{[a_1,b]}$ ist die gesuchte linear umparametrisierte lokale Geodäte $c_{\ol{p},\ol{q}}$. 
	\end{enumerate}
\end{beweis}
