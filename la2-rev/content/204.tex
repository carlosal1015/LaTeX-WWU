...
\subsection{Orthogonale Projektionen und Orthonormalbasen}
\label{sec:2.4}
...
\begin{minipage}{.7\textwidth}
	\begin{satz}[Satz von \textsc{Pythagoras}]
		\label{satz:4.7}
		Ist $\{v_1,\dots,v_l\}$ ein Orthogonalsystem, so gilt für alle $\lambda_1,\dots,\lambda_l \in \KK$: \index{Pythagoras}
		\[
...
\begin{beweis}
	Seien $\lambda_1, \dots, \lambda_l \in \KK$ mit $\sum_{i=1}^{l} \lambda_i v_i = 0$, so folgt mit Satz~\ref{satz:4.7}:
	...
	Da $v_i \neq 0$, folgt $\lambda_i = 0$ für alle $1 \leq i \leq l$. 
\end{beweis}
