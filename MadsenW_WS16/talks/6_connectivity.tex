...
\begin{proof}[{name={of \autoref{thm:connec_full}}}]
	...
	\begin{figure}[htb]
		...
		\begin{tikzpicture}
			...
			\draw[fill=gray!40] (3.3,-.3) .. controls +(12:.5) and +(180:.9) .. (3.9,.6) .. controls +(0:.9) and +(170:.5) .. (4.6,-.3);
	...
	\begin{figure}[hbt]
		...
		\begin{tikzpicture}
			...
			\draw[thick] (1,0) .. controls +(85:4) and +(183:5) .. (.3,-1.3) .. controls +(3:2) and +(225:4) .. (4,2) node[below right] {$a_i$};
			\draw[thick] (1,0) .. controls +(15:1) and +(225:3.5) .. (4,2.15) node[above] {$R(a_i)$};
			\draw[thick,dotted] (1,0) .. controls +(90:3.5) and +(183:4.5) .. (.3,-1.2) node[pos=.25,pin={[pin distance=4ex,pin edge={solid}]40:$L(a_i)$}]{};
...
\begin{proof}
	...
	\begin{figure}[htb]
		...
		\end{tikzpicture}\\[-1cm]
		\caption{A purely bad $2$-simplex, and vertex in its link/complement}\label{fig:purely-bad}
	\end{figure}
