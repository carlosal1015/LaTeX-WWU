...
\begin{proof}
	...
	\begin{itemize}
		\item If $b_0,b_1$ are in the different boundary components, this is true regardless of the fact, that $\sigma$ is purely bad:\marginnote{$\chi(S_{g,r}) = 2 - 2g -r$ and $\chi(S\setminus \sigma) = \chi(S)+ p + 1$}
		...
		Cutting along both arcs leaves us with a surface with the same number of boundary components as before.\todo{draw a picture?}
