...
\begin{sketch}
	...
	\end{figure}
	We get a decomposition\todo{definition of Star and Link}
	\[
...
\begin{proof}[{name={of \autoref{thm:connec_full}}}]
	...
	Because of \autoref{lem:d_dplus1} we may now assume that in the general case $\Delta$ has at most one point in each boundary component of $S$ to prove contractibility of $\mathcal{A}(S,\Delta)$. 
	We first notice, that $\mathcal{A}(S,\Delta)$ is not empty: 
	\begin{itemize}
		\item If $\Delta$ has at least two points, any arc connecting two points in different boundary components is non-trivial.
...
An edge is called \emph{pure} if both of its endpoints lie in either $\Delta_0$ or $\Delta_1$; it is called \emph{impure} otherwise.
Note that a priori this definition has nothing to do with the arcs we are interested in. 
