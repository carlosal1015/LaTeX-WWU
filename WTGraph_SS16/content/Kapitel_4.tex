...
und normiert $\mu_{\Lambda,\beta}$ zu einem Wahrscheinlichkeitsmaß.
 ...
 \underline{Existenz der freien Energie} \\
 ...
 \end{align}
 weil $\lambda_1 > \lambda_2$ \todo{missing part at Z lambDa beta h}
 \begin{align}
...
Leider ist das Unsinn, denn für fast alle $\sigma$ konvergieren die Reihen nicht. \todo{detla sigma} Das $H$ global existiert ist vielleicht auch mehr als wir wünschen. Wir wollen eigentlich 
\begin{align}
...
\begin{definition}
	...
	\end{enumerate}
\end{definition}
	Zunächst versuchen \todo{check def} wir die Situation zu verkleinern. 
...
\begin{enumerate}[a)]
	...
	\end{align}
	gegeben und für kleines $\beta$ spielt die Energie beinahe keine Rolle, d.h. \todo{missing nebenrechnung. Z schlange} $\mu_{\Lambda, \beta}^{\eta}$ ist eine Perkolation des Produktmaßes. 
...
\begin{beweis}
	...
	Wenn $\sigma_0 = -1$, dann wird der Ursprung $0$ unter $\mu_{\Lambda, \beta}^{+}$ von einer Kontur begrenzt, sodass die Spins innerhalb von $\gamma$ alle $-1$ sind. Also ist \todo{int: interior?}
	\begin{align}
