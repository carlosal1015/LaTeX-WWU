...
\begin{beispiel*}[{name=[Überlagerungen]}]
	...
	\begin{itemize}[itemsep=2pt]
	    ...
	    Ferner gilt $M^n = \sfrac{\widetilde{M}^n}{\pi_1(M^n)}$.\todo{RevChap 1}
	\end{itemize}
\end{beispiel*}
...
\begin{lemma}[label=lem:132]
    ...
    \begin{enumerate}[1)]
        \item Ist $\widetilde{G}$ eine zusammenhängede Mannigfaltigkeit und $\pi \colon \widetilde{G} \to G$ eine Überlagerungsabbildung, so besitzt $\widetilde{G}$ genau eine Liegruppenstruktur, sodass $\pi$ ein Liegruppenhomomorphismus ist.
        \item Ein Liegruppenhomomorphismus $\Phi \colon \widetilde{G} \to G$ mit $\widetilde{G}$ zusammenhängend ist eine Überlagerung genau dann, wenn das Differential $(\mathd \Phi)_{\tilde{e}} \colon \Tmap_{\tilde{e}} \widetilde{G} \to \Tmap_e G$ ein Isomorphismus ist.
    \end{enumerate}
\end{lemma}
\begin{beweis}
    \begin{enumerate}[(1)]
        \item Sei $\tilde{e} \in \pi^{-1}(e)$ fest gewählt.
        Überlagerungstheorie impliziert, dass die stetigen Funktionen $m \colon G \times G \to G$, $^{-1} \colon G \to G$ entsprechende Lifts besitzen.\footnote{siehe zum Beispiel \textsc{Stöcker-Zischer} \emph{Algebraische Topologie} \cite[Abschnitt 6.2]{stockerAlgTop}}
        Die Eindeutigkeit ist durch die Wahl von $\tilde{e}$ gegeben [Klar, falls $\widetilde{G}$ einfach zusammenhängend, sonst \enquote{kleines} Argument].
