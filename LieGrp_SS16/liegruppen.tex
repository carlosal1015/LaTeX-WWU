...
Ist $G=S^3$, so wurde in den Übungen (Aufgabe 3)\todo{eventuell \TeX{}en?} die adjungierte Darstellung von $G$ bestimmt:
\[
...
\begin{beispiel*}[{name=[exakte Sequenzen und Auflösbarkeit]}]
	...
	\begin{itemize}
		...
		\[
			\mathrm{o}(n,\mathbb{K}) = \set[\big]{A \in \Mat(n,\mathbb{K}) \given A_{ij}=0 \,\, \forall 1 \le i<j \le n}
		\]
		...
		\[
			\mathrm{n}(n,\mathbb{K}) = \set[\big]{A \in \mathrm{o}(n,\mathbb{K}) \given A_{ii}=0 \enspace i=1, \ldots ,n}
		\]
		ist eine nilpotente Liealgebra mit $\mathrm{n}(n,\mathbb{K})_{n-1}=0$.
		\item Die Liealgebra 
		\[
		...
		\[
			...
			\end{tikzcd} 
		\]
		$1$-dimensionale Liealgebren sind abelsch und somit nilpotent, also auch die beiden äußeren Ideale.
		Damit ist \autoref{lem:213} 6) nicht richtig für nilpotente Algebren.\todo{RevChap 2}
	\end{itemize}
\end{beispiel*}
