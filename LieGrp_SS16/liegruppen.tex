...
\begin{satz}
	...
	\begin{enumerate}[1)]
		...
		\item $\sfrac{\Aut(\mathfrak{g})}{\Int(\mathfrak{g})}$ ist diskret (fast nur innere Automorphismen $\leadsto$ Konjugation).
	\end{enumerate}
\end{satz}
\begin{beweis}
	Vorüberlegungen:
	\begin{enumerate}[i)]
		\item Ist $\mathfrak{a}$ ein Ideal von $\mathfrak{g}$, so auch 
		\[
		...
		\]
		\item Ist $\mathfrak{a}$ ein Ideal, so ist $\mathfrak{a} \cap \mathfrak{a}^\bot = \set*{0}$.
		...
		\item $\mathfrak{g} = \mathfrak{a} \oplus \mathfrak{a}^\perp$ (gilt viel allgemeiner!)
	\end{enumerate} 
\end{beweis}
% section 22 (end)
% chapter 2 (end)
