...
Ist $\Phi \colon G \to \GL(n,\mathbb{K})$ injektiv, so besitzt die Gruppe $G$ die \bet{treue Darstellung}\index{Darstellung!treue} $\Phi(G)$ in $\GL(n,\mathbb{K})$.
...
\begin{beispiel*}[{name=[Lieuntergruppen]}]
	...
	\begin{itemize}
		...
		\begin{align}
			...
			&= \exp(0) =E_n
		\end{align}
		Dies folgt mit $\exp(X+Y)= \exp(X) \exp(Y)$, falls $[X,Y]=0$. Siehe \cref{sec:funktionalgleichung} für eine allgemeine Herleitung dieser Funktionalgleichung für Liegruppen.
	\end{itemize}
\end{beispiel*}
...
\begin{beweis}
	\begin{enumerate}[1)]
		...
		Die Eigenschaften $y \in \mathfrak{h}$, $y \in \mathfrak{h}^\bot$ und $\norm*{y}=1$ widersprechen sich!
...
Ist in obiger Situation $H$ \emph{nicht} einfach zusammenhängend, so erhält man einen Homomorphismus $\widetilde{\Phi} \colon \widetilde{H} \to G$, wobei $\widetilde{H}$ eine universelle Überlagerung von $H$ ist.
Dies liefert genau dann einen Homomorphismus $\Phi \colon H \to G$, wenn $\ker \pi \subset \ker \widetilde{\Phi}$ ist, wobei $\pi \colon \widetilde{H} \to H$ die Überlagerungsabbildung bezeichnet.
...
Dann ist die induzierte Abbildung $\widetilde{\Phi} \colon \widetilde{H} \to \widetilde{H}$ injektiv, aber $\pi \colon \widetilde{H} \to H$ muss nicht injektiv sein.
Ein konkretes Beispiel wäre die universelle Überlagerung von $\SL(n,\mathbb{R})$.

\begin{satz}[{name=[{Liegruppen-Liealgebren-Zusammenhang im einfach zusammenhängenden Fall}]},label=satz:135]
	Es gilt
	\begin{enumerate}[1)]
...
\begin{beweis}
	\begin{enumerate}[1)]
		...
		\item Mit $\tilde{G}$ als universelle Überlagerung folgt dies direkt aus \autoref{lem:133}.\qedhere
	\end{enumerate}
\end{beweis}
...
\begin{beispiel*}[{name=[Überlagerung der speziellen linearen Gruppe]}]
	...
	\]
	Wir führen folgende Multiplikation auf $M^3$ ein:\marginnote{wurde in den Übungen detailliert durchgerechnet}
	\[
...
\begin{lemma}[label=lem:143,{name=[Eigenschaften der Exponentialabbildung]}]
	...
	\begin{enumerate}[(1),itemsep=0pt]
		\item Für alle $v \in \Tmap_eG$ ist $\Phi_v(t)=\exp(tv)$ eine Einparametergruppe von $G$ mit $\Phi'_v(0)=v$.
...
\begin{bemerkung*}[{name=[weitere Eigenschaften der Exponentialabbildung]}]
	...
	\begin{enumerate}[1)]
		...
		\item Es gilt $\exp(x)^{-1} = \exp(-x)$ (nach der Funktionalgleichung aus \cref{sec:funktionalgleichung}).
...
\begin{beweis}
	\begin{enumerate}[1)]
		...
		Somit existieren linear unabhängige $v_1, \ldots, v_k \in \mathfrak{g}$ mit
		\[
...
\begin{beweis}
	...
	\)
	in $t=0$ liefert wegen der Produktregel
	\[
...
\begin{beispiel*}[{name=[Automorphismen einer abelsche Liealgebra]}]
	...
	Nichttriviale innere Automorphismen existieren in einer solchen auch nicht.
\end{beispiel*}
...
Achtung! $S^1_r \subset T^2 = S^1 \times S^1$ für $r \in \mathbb{R}\setminus \mathbb{Q}$ ist Normalteiler mit $\overline{S^1_r}= T^2$, die Eigenschaft einfach zusammenhängend zu sein ist also von zentraler Wichtigkeit.
...
\begin{beispiel*}[{name=[spezielle orthogonale Gruppe]}]
	...
	\]
	gilt.
	Wir erhalten für eine ONB $\set*{b_1, \ldots ,b_N}$ von $\mathfrak{so}(n)$, $N=\frac{1}{2}(n-1)n$\marginnote{allgemein gilt:\newline $\mathcal{B}(X,Y)= (n-2) \tr(XY)$ }
	\begin{align}
		\mathcal{B}(v,w) = \tr \enbrace*{\ad_v \circ \ad_w} = \sum_{i=1}^{N} \skal[\big]{(\ad_v \circ \ad_w)(b_i)}{b_i}_{\mathfrak{so}(n)} 
		\stackrel{!}{=} -\sum_{i<j} \skal[\big]{\benbrace*{w,e_i \wedge e_j}}{ \benbrace*{v, e_i \wedge e_j}}
	\end{align}
	Somit ist $\mathcal{B}(e_k \wedge e_l, e_k \wedge e_l)=-2(n-2)$ und für $\set*{k,l} \neq \set*{\tilde{k},\tilde{l}}$
	\[
...
\begin{lemma}[label=lem:213,{name=[Grundlegendes zu Nilpotenz und Auflösbarkeit]}]
	Sei $\mathfrak{g}$ eine Liealgebra, die nilpotent bzw. auflösbar in $k$ Schritten ist.
	Dann gilt:
	\begin{enumerate}[1),itemsep=0pt]
...
\begin{beweis}
	...
	Sei $\mathfrak{h}$ eine echte Unteralgebra maximaler Dimension (d.h. $\le n$) von $\mathfrak{g}$\marginnote{1-dimensionlae Unterräume sind Unteralgebren}.
...
\begin{beispiel*}[{name=[Komplexifizierungen]}]
	\begin{itemize}
		...
		\item $\SL(2,\mathbb{R})$ ist eine nicht-kompakte Liegruppe.
	\end{itemize}
\end{beispiel*}
...
\begin{enumerate}[1)]
	...
	\]
	\item Für die Killingform gilt: $\mathcal{B}_\mathbb{C} \enbrace*{v_1 \otimes \lambda_1, v_2 \otimes \lambda_2} = \mathcal{B}_\mathbb{R}(v_1,v_2) \cdot \lambda_1 \lambda_2 \in \mathbb{C}$
\end{enumerate}
...
\begin{beweis}
	...
	Wir wählen eine Basis $a_1, \ldots ,a_k, b_1, \ldots ,b_s$ von $\mathfrak{g}$ mit $\mathfrak{a} = \langle a_1, \ldots , a_k\rangle_\mathbb{K}$.
...
\begin{beweis}
	...
	\begin{enumerate}[1),itemsep=0pt]
		...
		Dann ist $\mathfrak{a} \cap \mathfrak{g}_i$ entweder $0$ oder $\mathfrak{g}_i$.
		...
		Im ersten Fall würde dann $\mathfrak{g}$ abelsch und damit die Killingform trivial sein, was im Widerspruch zu Cartans Kriterium steht.
...
\begin{definition}[{name=[{kompakte Liealgebra}]}]
	Eine reelle Liealgebra $\mathfrak{g}$ nennt man \Index{kompakt}, falls es eine kompakte Liegruppe $G$ gibt, für die $\Tmap_e G \cong \mathfrak{g}$ ist. 
\end{definition}
...
\begin{beweis}
	\begin{enumerate}[1)]
		...
		Zu zeigen ist, dass $(\mathd R_h)_g \colon \Tmap_g G \to \Tmap_{R_h(g)} G$ eine Isometrie ist:
		...
		Dann gilt
		\begin{align}
			g_\bi \enbrace*{(\mathd R_g)_e \cdot X, \enbrace*{\mathd R_g}_e \cdot Y}_g = \skal[\big]{ \enbrace*{\mathd L_{g^{-1}}}_g \enbrace*{\mathd R_g}_e \cdot X}{\cdots}
...
\begin{lemma}[{name={Weyl}},label=lem:235]
	...
	\[
		\abs*{\pi_1(G)} < \infty
	\]
	Damit ist auch jede weitere Liegruppe mit Liealgebra $\mathfrak{g}$ kompakt.\marginnote{Zeige, dass $\widetilde{G}$ kompakt ist, folgt aus: Basis ist Hausdorffsch, kompakt und Überlagerung endlich blättrig}
\end{lemma}
\begin{beweis}
...
\begin{beweis}
	...
	Somit ist $T$ abgeschlossen, also kompakt, das heißt $T = S^1 \times \ldots \times S^1$, $T$ ist also ein Torus.
...
\begin{beweis}
	...
	Folglich ist $A \cong \sfrac{\mathbb{Z}}{m \mathbb{Z}} \times T^k$ für ein $k \ge 1$, $m \ge 1$ minimal mit $g^m \in A_e$.\marginnote{siehe \cref{sec:dichte_teilmenge_torus}}
...
\begin{beweis}
	...
	Wir setzen
	\[
		Z_v \coloneqq \set[\big]{\tilde{g} \in G \given \Ad(\tilde{g}) v = v}
	\]
	...
	Dann gilt für den Tangentialraum des Orbits $\Ad(G)v = \Ad(G)\tilde{v}$
	\[
	...
	\[
		\skal[\big]{\benbrace*{w,\tilde{v}}}{t} = \skal[\big]{w}{\benbrace*{\tilde{v}, t}} \stackrel[\mathfrak{t} \text{ abelsch} ]{\benbrace*{\tilde{v},t}=0}{=\joinrel=\joinrel=\joinrel=}0
	\]
...
\begin{beispiel*}[{name=[maximale Tori der speziellen unitären Gruppe]}]
	...
	\]
	Alle adjungierten Orbiten sind von der Form $\sfrac{\SU(n)}{\SU(n_1) \times \ldots \times \Un(n_k)}$ mit\marginnote{das Ganze sollte man mal nachrechnen}
	\[
...
\begin{beweis}
	...
	\begin{enumerate}[(i)]
		...
		Dann gilt
		\mapdef{\varphi_{\tilde{g}} \circ \varphi_g^{-1} \colon \varphi_g \enbrace*{\sfrac{U_{\tilde{g}}}{H} \cap \sfrac{U_g}{H}}}{\varphi_{\tilde{g}} \enbrace*{\sfrac{U_{\tilde{g}}}{H} \cap \sfrac{U_g}{H}}}{d}{\tilde{g}^{-1}g d h_d}{}
...
\begin{lemma}[label=lem:316,{name=[$G$-äquivarianter Diffeo mit Standgruppe]}]
	...
	\]
	die \Index{Isotropiegruppe} der Wirkung. 
...
\begin{beispiel*}[{name=[homogene Räume]},label=bsp:homSpaces]
	...
	\begin{enumerate}[1)]
		...
		\item Die unitäre Gruppe $\Un(n) = \SO(2n) \cap \Mat(n,\mathbb{C})$\marginnote{die Schreibweise als Schnitt wurde in Aufgabe 11 gezeigt} operiert ebenfalls transitiv auf $S^{2n-1}$.
		...
		\]
		Jede schiefsymmetrische Matrix kann mittels Konjugation (durch $\On(2n)$) auf die Form 
		\[
			\begin{pmatrix}
				0 & - \alpha_1 & & & & \\
				\alpha_1 & 0 & & & & \\
...
\begin{itemize}[itemsep=1pt]
	...
	\item $\mathd(\exp_p)_{0_p} \colon \Tmap_p M^n \to \Tmap_p M^n$ ist gleich $\id_{\Tmap_p M^n}$ und nach dem Umkehrsatz ist $\exp_p$ ein lokaler Diffeomorphismus.
\end{itemize} 
...
\begin{beweis}
	...
	Da $\On \enbrace*{\Tmap_p M^n,g_p}$ kompakt ist, können wir also annehmen, dass\marginnote{sonst Übergang zu Teilfolge}
	\[
...
\begin{beweis}
	...
	\end{align}
	für alle $g \in G$, also folgt $\overline{L}_h \circ \pi = \pi \circ i_h$.
...
\begin{beispiel*}[{name=[Killingvektorfelder]}]
	\begin{itemize}
		...
		Es gilt 
		\[
			X^*(gH) = \diffd{}{t}\Big|_{t=0} \Underbracket{\overline{L}_{\exp(tX)} \cdot gH}{= \exp(t X) \cdot g \cdot H = \pi \enbrace*{\exp(tX) \cdot g}} = (\mathd \pi)_g \cdot \Underbracket{\diffd{}{t}\Big|_{t=0} \exp(tX) \cdot g }{= X^R(g)}
		\]
...
\begin{satz}[label=satz:324,{name=[Formel für Krümmungstensor]}]
	Sei $\enbrace*{\sfrac{G}{H},g}$ ein homogener Raum und $\mathfrak{g} = \mathfrak{h} \oplus \mathfrak{m}$ eine $\Ad(H)$-invariante Zerlegung von $\mathfrak{g}$.
	Dann gilt für $X,Y \in \mathfrak{m}$ die folgende Identität:\marginnote{Achtung: Ab diesem Satz bin ich mir im gesamten Abschnitt 3.2 nicht sicher, ob ich alles richtig mitgeschrieben habe, gerade kleine Indizes kann man leicht übersehen!}
	\begin{align}
...
\begin{lemma}[{name=[Konstruktion von Z]}]
...
\begin{satz}[{name=[Formel für die Ricci-Krümmung gleicher Vektoren]},label=satz:327]
...
\begin{beweis}
	...
	Insbesondere gilt $\skal*{\ad(Z)X}{X} =0$, womit der letze Term der Formel aus \autoref{satz:327} verschwindet.
	Ferner gilt
	\begin{align}
...
Eine weitere Krümmungsgröße einer Riemannschen Mannigfaltigkeit $(M^n,g)$ ist die \Index{Skalarkrümmung}:
\mapdef{\operatorname{Scal}(g) \colon M^n}{\mathbb{R}}{p}{\sum_{i=1}^n \ric(g)_p (X_i^p, X_i^p)}{}
...
Wir wenden uns nun dem Raum $\mathcal{M}_G$ aller $G$-invarianten Metriken auf einem Riemannschen homogenen Raum $\sfrac{G}{H}$ zu.
Wir hatten bereits gesehen, dass $\mathcal{M}_G$ mit dem Raum der $\Ad(H)$-invarianten Skalarprodukte auf einem $\Ad(H)$-invarianten Komplement $\mathfrak{m}$ von $\mathfrak{h}$ in $\mathfrak{g}$ identifiziert werden kann.\marginnote{siehe \autoref{kor:3113}}
...
\mapdef{\Phi \colon \GL(n,\mathbb{R}) \times P(n)}{P(n)}{(A,P)}{A P A^T}{}
...
\mapdef{\sskal*{\cdot}{\cdot}_p \colon \Tmap_p P(n) \times \Tmap_p P(n)}{\mathbb{R}}{(V,W)}{\tr \enbrace*{P^{-1} V P^{-1} W}}{}
...
\mapdef{\exp \colon \Tmap_I P(n) }{P(n)}{V}{\exp(V)}{}
ein Diffeomorphismus.\marginnote{Aus $P(n) \cong \mathbb{R}^{1/2 n(n+1)}$ und $R_{\sskal*{\cdot }{\cdot }} \le 0$ folgt dies auch mit Hadamard-Cartan}
...
\begin{bemerkung*}[{name=[Teilmenge der positiv definiten Matrizen]}]
	\begin{itemize}
		...
		\[
			\gamma_V(t) = \exp(t V) 
		\]
		für $V \in \Tmap_I P(n)^1$ eine Geodätische in $P(n)^1$ (\Index{total gedätische Untermannigfaltigkeit})
		\item $P(n)^1 = \sfrac{\SL(n,\mathbb{R})}{\SO(n)}$. Dies ist ein symmetrischer Raum vom nicht kompakten Typ.
	\end{itemize}
\end{bemerkung*}

\begin{satz}[label=satz:3210,{name=[Raum M als Bild der Exponentialabbildung]}]
...
Man nennt $\Ad(H)$-Module $\mathfrak{m}_i, \mathfrak{m}_j$ \emph{äquivalent}, falls es Isomorphismus $J \colon \mathfrak{m}_i \to \mathfrak{m}_j$ existiert mit 
\[
	\begin{tikzcd}
...
Ein irreduzibler Modul $V$ einer linearen Darstellung einer kompakten Liegruppe $K$ (bei uns $K = \Ad(H)$ operiert auf dem Modul $\mathfrak{m}$) ist vom \bet{reellen}, \bet{komplexen}, \bet{quaternionalen} Typ, falls\marginnote{siehe \cite{BrotD}}
\[
...
\Index{Schurs Lemma:} Sei $K$ eine kompakte Liegruppe und $\rho \colon K \to \GL(V)$ eine irreduzible Darstellung von $K$ (auf Vektorraum $V$).
...
\begin{beispiel*}[{name=[Gestalt von $P$ für Zerlegung in zwei Summanden]}]
...
\begin{itemize}
	...
	\end{align}
\end{itemize}
Wie früher definieren wir\marginnote{zum Vergleich: Es war $\ad(X)(Y)= \benbrace*{X,Y}$}
...
\begin{satz}[{name=[Verbesserung der Formel für die Ricci-Krümmung]}]
...
\begin{bemerkung*}[{name=[topologische Eigenschaften von $V(n)$]}]
	\begin{itemize}
...
\begin{lemma}[{name=[Eigenschaften der linearen Darstellung $\pi$]}]
...
\begin{lemma}[{name=[Formel mit der Momentenabbildung]}]
...
Bezeichnung $\Ric_\mu \coloneqq -\frac{1}{2} \mathcal{B}_\mu + M_\mu - \ad_\mu(Z_\mu)$ \Index{Ricci-Endomorphismus} von $\mu \in V(n)$.
