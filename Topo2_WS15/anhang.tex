...
\begin{beweis}[name={\cite[S. 199 ff]{Hatcher}}]
	...
	\begin{enumerate}[(i)]
		...
		$i^*$ ist surjektiv, da $i^*$ eine Kokette auf ihre Einschränkung auf singuläre $n$-Simplizes in $A$ abbildet und man eine Kokette in $C^n(A)$ durch $0$ offensichtlich auf singuläre $n$-Simplizes auf ganz $X$ fortsetzten lässt.\marginnote{Man kann hier auch die Linksexaktheit von $\Hom(-,V)$ anwenden}
...
\subsection{Kurze exakte Folge im Künneth-Theorem} % (fold)
...
\subsection{Azyklische Modelle} % (fold)
...
Weiter gelte
\begin{itemize}
	\item $F$ ist \emph{frei} auf $\mathcal{M}$, das heißt für jedes $n\ge 0$ gibt es eine Teilmenge $\mathcal{M}_n \subset \mathcal{M}$, sodass es für jedes Modell $M \in \mathcal{M}_n$ ein Element $m_M \in F(M)_n$ gibt,\marginnote{$m_M$ ist in singulären Fall die Identität als singulärer Simplex} sodass $\set*{F(f)(m_M) \given f \colon M \to X, M \in \mathcal{M}_n}$ für jedes Objekt $X$ in $\mathcal{C}$ eine Basis von $F(X)_n$ bildet.
...
\begin{beweis}
	...
	\begin{enumerate}[a)]
		...
		Da $F$ frei ist, können wir also $\phi_{n,X} \colon  F(X)_n \to G(X)_n$ durch\marginnote{$f \colon M \to X$}
		\[
	...
	\end{enumerate}
\end{beweis}
% subsection azyklische_modelle (end)
