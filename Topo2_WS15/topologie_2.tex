...
\begin{definition}[{name=[singuläre Kohomologie mit kompaktem Träger]}]
	Sei $X$ ein topologischer Raum. Dann definieren wir die\marginnote{$H^*_c$ ist \emph{kein} Funktor von $\TOP$, um einen Funktor zu erhalten muss man sich auf eigentliche Abbildungen beschränken, d.h. stetige Abbildungen, sodass Urbilder kompakter Mengen wieder kompakt sind!} \bet{singuläre Kohomologie von $X$ mit kompaktem Träger}\index{Kohomologie mit kompaktem Träger} durch
	\[
...
\begin{satz}[label=satz:homol_tensor,{name=[{Homologie des Tensorproduktes von Kettenkomplexen}]}]
...
\begin{beweis}
	...
	\begin{figure}[hbt]
		\centering{
			\[
				\begin{tikzcd}[row sep=large]
					C_{p} \otimes D_{q+1} \dar["\id \otimes \partial^D"] & C_{p+1} \otimes D_{q+1} \dar["\id \otimes \partial^D"] \\
	...
	Nun ist $\begin{tikzcd}[cramped,sep=small] H_p(C_*) & C_p \lar[two heads] & C_{p+1} \lar[{Hooks[left]}-{Computer Modern Rightarrow}] \end{tikzcd}$ eine freie und damit auch projektive Auflösung von $H_p(C_*)$, also gilt für die einzigen beiden Moduln der $2$-Seite\marginnote{Homomorphiesatz}
	\begin{align}
