...
\begin{definition}[{name=[{injektive Auflösung}]}]
	...
	\[
		\begin{tikzcd}
			0 \rar & M \rar[hook] & I_0 \rar["\partial^0"] & I_1 \rar["\partial^1"] & \ldots 
		\end{tikzcd}
	\]
	mit $I_i$ injektiv für alle $i$.
\end{definition}
...
\begin{lemma}[{name=[{exakte Folgen mit Moduln der $2$-Seite}]}]
...
\end{proposition}
\begin{beweis}
	Nach \autoref{satz:spektralfolge_filtrierung} haben wir die folgenden Isomorphismen\marginnote{die horizontale Filtrierung unterscheidet sich quasi nur durch eine Spiegelung ander Diagonalen von der vertikalen Filtrierung}
	\[
...
\begin{satz}[{name=[{Spektralfolge mit Tor auf der 2-Seite}]}]
...
\begin{beweis}
	...
	\]
	und daher $^v E^2_{p,q}=H_p \enbrace[\big]{P_* \otimes_R H_q(C_*)}= \Tor_p^R \enbrace[\big]{V, H_q(C_*)}$.\todo{Revision 17}
\end{beweis}
...
\begin{satz}[{name=[{Universelles Koeffizienten-Theorem für Homologie}]},label=satz:univ_koeff_homo_space]
...
\end{satz}
\todo[inline]{Hier ist eventuell HIR nötig}
\begin{beweis}
