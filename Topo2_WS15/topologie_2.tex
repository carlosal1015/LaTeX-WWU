...
\begin{bemerkung}[{name=[Begründung des Namens \enquote{Schnittform}]}]
	...
	Seien für $i=0,1$ $N_i$ geschlossene $K$-orientierte\marginnote{$K$ Körper} $k_i$-Mannigfaltigkeiten mit Fundamentalklassen $\mu_i \in H_{k_i}(N_i;K)$.
...
\begin{beispiel}[{name=[{Signatur des komplex projektiven Raumes}]}]
	Sei $\alpha \in H^2(\mathbb{CP}^{2k})$ ein Erzeuger.\marginnote{vgl. \autoref{satz:kohomo_ring_projektiv} (ii)}
...
\end{satz}
\begin{beweis}
	Sei $L :=  \im \enbrace[\big]{i^* \colon H^{2k}(W;\mathbb{R}) \to H^{2k}(\partial W;\mathbb{R})}$. Es gilt
	\begin{enumerate}[a)]
...
\begin{definition}[{name=[{Hom von Kettenkomplex und Kokettenkomplex}]}]
	...
	\]
	und die Randabbildung durch\marginnote{graduierter Kommutator}
	\(
