...
\begin{definition}[{name=[Kokettenkomplex und Kohomologie]}]
	Sei $R$ ein Ring. 
	Ein \bet{$R$-Kokettenkomplex}\index{Kokettenkomplex} $(C^*,d^*)$ ist eine Folge von $R$-Moduln $(C^n)_{n}$ zusammen mit $R$-linearen Abbildungen $d^n \colon C^n \to C^{n+1}$, sodass $d^{n+1} \circ d^n =0$ gilt. 
	Der \bet{$n$-te Kohomologiemodul}\index{Kohomologiemodul} von $(C^*,d^*)$ ist definiert als 
	\[
...
\begin{bemerkung}[{name=[Dualer Kokettenkomplex und Umwandlung von Kettenkomplexe in Kokettenkomplexe]}] \leavevmode
	\begin{enumerate}[i)]
		\item Sei $(C_*,d_*)$ ein $R$-Kettenkomplex und $V$ ein $R$-Modul. Dann erhalten wir einen $\mathbb{Z}$"=Kokettenkomplex\marginnote{$R$-Kokettenkomplex im kommutativen Fall} $(C^*,d^*)$ durch 
		\[
			C^n \coloneqq \Hom_R(C_n,V) 
		\]
		und $d^n \colon C^n \to C^{n+1}$ definiert durch $\alpha \mapsto \alpha \circ d_{n+1}$. 
		Dieser Kokettenkomplex $(C^*,d^*)$ heißt der \bet{$V$-duale}\index{Kokettenkomplex!$V$-dualer Kokettenkomplex} Kokettenkomplex zu $(C_*,d_*)$. 
		Ist $V=R$, so nennen wir ihn auch kürzer \Index{dualer Kokettenkomplex}.
...
\begin{bemerkung}[{name=[Isomorphie des singulären Kettenkomplexes]}]
	...
	\emph{Siehe auch \cref{sub:einschrankung_und_erweiterung_der_skalare}.}
\end{bemerkung}
...
\begin{definition}[{name=[kontravarianter Funktor]}]
	...
	Ein \Index{kontravarianter Funktor}  $F \colon \mathcal{C} \to \mathcal{D}$ ordnet jedem Objekt $C$ in $\mathcal{C}$ ein Objekt $D$ in $\mathcal{D}$ zu und jedem Morphismus $f \colon C \to C'$ einem Morphismus $F(f) \colon F(C') \to F(C)$ in $\mathcal{D}$ zu.\marginnote{\enquote{Pfeile umdrehen}}
	Dabei muss gelten:
	\begin{enumerate}[i),noitemsep]
		\item $F(\id_C)= \id_{F(C)}$
		\item Für $\begin{tikzcd}[cramped] C \rar["f"] & C' \rar["f'"] & C'' \end{tikzcd}$ gilt $F(f' \circ f) = F(f) \circ F(f')$. 
	\end{enumerate}
	Kürzer ist ein \emph{kontravarianter} Funktor $F \colon \mathcal{C} \to \mathcal{D}$ dasselbe wie ein \emph{kovarianter} Funktor $\mathcal{C}^\op \to \mathcal{D}$.
\end{definition}
...
\begin{bemerkung}[{name=[Angabe des Verbindungshomomorphismus]},label=bem:verbindungshomo]
	Sei $(X,A)$ ein Paar von topologischen Räumen. 
	Der Verbindungshomomorphismus $\partial \colon H^n(A;V) \to H^{n+1}(X,A;V)$ kann wie folgt beschrieben werden:\marginnote{Schlangenlemma/\newline{}Diagrammjagd} 
	Sei $\alpha \colon C_n(A) \to V$ ein Kozykel. 
	Setze $\alpha$  durch $\hat{\alpha}(\sigma)=0$ für $\sigma \colon \abs{\Delta^n} \to X$, $\im \sigma \nsubseteq A$ zu $\hat{\alpha} \colon C_n(X) \to V$ fort. 
	Dann ist 
	\[
...
\begin{bemerkung}[{name=[Grundlegende Eigenschaften der Paarung]}]
	...
	\begin{itemize}[itemsep=0pt]
		...
		\item Für $r \in \mathbb{Z}$, $x,x' \in H_n(X,A)$ und $\xi,\xi' \in H^n(X,A;V)$ gelten folgende Regeln
		\[
...
\noindent Um den Satz beweisen zu können, benötigen wir zunächst zwei Hilfslemmata:

\begin{lemma}[label=lem:untergrp_frei,{name=[Untergruppen freier abelscher Gruppen sind frei]}]
	Untergruppen freier abelscher Gruppen sind frei.\marginnote{Dies gilt auch für $R$-Moduln über einem Hauptidealring, siehe auch \cref{sub:untermod_frei}. Damit folgt \autoref{eig_kohomo_to_hom_homo} auch für HIR.}
...
\begin{bemerkung}[{name=[{Verschärfung von \autoref{kor:26} mit Q-Homomorphismen}]}]
	Es gilt sogar $H^n \enbrace*{X,A;\mathbb{Q}} \cong \Hom_\mathbb{Q} \enbrace[\big]{H_n(X,A;\mathbb{Q}),\mathbb{Q}}$ mittels \emph{Erweiterung der Skalare} und dem \emph{universellen Koeffiziententheorem}, siehe \cref{sub:einschrankung_und_erweiterung_der_skalare} und \autoref{satz:hir_tensorprod_homologie}.
\end{bemerkung}
\newpage
% section 2 (end)
...
\begin{beweis}
	...
	\begin{align}
		...
		\shortintertext{da $p^*_X(\hat{\alpha}) \cupp p_Y^*(\beta)$ eine Fortsetzung von $p_A^* (\alpha) \cupp p_Y^*(\beta)$ ist. Weiter gilt nach \autoref{lem:eig_cup}}
...
\begin{beweis} Wir betrachten folgendes Diagramm
	...
	Insbesondere spaltet die Sequenz und bleibt somit exakt nach Anwenden des Funktors ${-}\otimes H^p(X;R)$.\marginnote{dieser Funktor ist stets rechtsexakt}
...
\begin{bemerkung}[{name=[{Angabe der Abbildung in \ref{sec3:prop_Hp-1}}]},label=angabe_iso_s1]
	...
	\]
	Dies macht man sich durch sorgsames Durchgehen der obigen Proposition klar.
\end{bemerkung}
% section 3 (end)
\newpage
...
\begin{bemerkung}[{name=[Cup-Produkt auf $C^*$ nicht graduiert kommutativ]}]
	...
	Zu zeigen: Für den Kohomologiering gilt dies allerdings schon!
\end{bemerkung}

\begin{lemma}[{name=[Spiegelung von singulären Simplizes]},label=spiegelung_simplex]
	Für $\sigma \colon \abs{\Delta^p} \to X$ sei die \bet{Spiegelung}\index{Spiegelung eines singulären Simplex} $\overline{\sigma} \colon \abs{\Delta^n} \to X$ definiert durch 
	\[
...
\begin{proposition}[label=prop:H0_kettenhomotop,{name=[Nullkettenhomotopie von natürlichen Transformationen]}]
	Sei $\tau_* \colon C_*^\sing \to C_*^\sing$ eine natürliche Transformation durch Kettenabbildungen.\marginnote{Alternativ mit \cref{sub:azyklische_modelle}, Methode der azyklischen Modelle}
...
\begin{beispiel}[{name=[Polynomring und äußere Algebra]}]
	Sind alle $\abs*{a_i}$ gerade, so ist $\Lambda_R \benbrace*{a_1,\ldots,a_n}$ der Polynomring\index{Polynomring} über $R$ in den Variablen $a_1, \ldots ,a_n$.
...
\begin{beispiel}[{name=[Kohomologieklassen des Torus]}]
	...
	\[
		\eta= \!\smashoperator[r]{\sum_{i_1<\ldots <i_k}}\,\, r_{i_1,\ldots ,i_k} \cdot \xi_{i_1} \cupp \ldots \cupp \xi_{i_k}
	\]
\end{beispiel}
% section 4 (end)
\newpage
...
\begin{frage}[{name=[Isomorphismen in Homologie und Kettenhomotopieäquivalenz]}]
...
\begin{frage}[{name=[Isomorphismus in Homologie im Satz über kleine Simplizes]}]
...
\begin{beispiel}[{name=[Beispiel eines azyklischen Kettenkomplex]}]
	...
	Damit ist die Antwort auf die erste Frage im Allgemeinen \enquote{Nein}.
	Unter zusätzlichen Forderungen an die Kettenmoduln ist die Antwort allerdings \enquote{Ja}, wie wir in diesem Kapitel sehen werden.
	Diese zusätzliche Forderung ist die folgende:
\end{beispiel}
...
\begin{definition}[{name=[Kegel und Einhängung]}]
	Sei $f \colon C_* \to D_*$ eine Kettenabbildung.\marginnote{Für $f \colon X \to Y$ ist $C_*(\Keg(f)) \cong \Keg(f_*)_*$ als Kettenkomplexe} 
...
\begin{lemma}[{name=[{Isomorphismus induziert genau dann, wenn der Kegel azyklisch ist}]},label=lem:iso_homo_keg_azyklisch]
	Eine Kettenabbildung $f\colon C_* \to D_*$ induziert genau dann einen Isomorphismus in Homologie, wenn $\Keg(f)_*$ azyklisch ist.
\end{lemma}
\begin{beweis}
...
\begin{korollar}[label=korr:ausschneidung_sing_koho,{name=[{Ausschneidung für Kohomologie}]}]
	Singuläre Kohomologie erfüllt das Ausschneidungsaxiom.\marginnote{vgl. \autoref{satz:110}}
\end{korollar}
\begin{beweis}
...
\begin{lemma}[{name=[Produktregel für das Cap-Produkt]},label=lem:cap_prod]
...
Wir wenden uns nun einmal mehr der Topologie von Mannigfaltigkeiten zu und erinnern uns dazu an das letzte Kapitel aus Topologie \RM{1} über die \emph{Fundamentalklasse} einer Mannigfaltigkeit:
...
\begin{beweis}
	...
	\begin{align}
		...
		\intertext{Da $M$ homotopieäquivalent zu einem CW-Komplex ist, ist $\dim_{\mathbb{F}_2} H_k(M; \mathbb{F}_2)<\infty $ und insbesondere gilt $\dim_{\mathbb{F}_2} H_k(M;\mathbb{F}_2) = \dim_{\mathbb{F}_2} \Hom_{\mathbb{F}_2} \enbrace*{H_k(M; \mathbb{F}_2),\mathbb{F}_2} = \dim_{\mathbb{F}_2} H^k(M; \mathbb{F}_2)$, sodass folgt}
...
\begin{erinnerungA}[{name=[Orientierungsklassen für kompakte Teilmengen]},label=errinnerung:Orientierung_kompakteTM]
	Sei $M$ eine (nicht notwendig kompakte) $n$-Mannigfaltigkeit und $K \subseteq M$ eine kompakte Teilmenge. Dann gilt nach Lemma 16.11 aus Topologie \RM{1}.:
	\begin{enumerate}[(i),itemsep=0pt]
...
\begin{beweis}
	...
	Wir haben nun das folgende kommutative Diagramm\marginnote{Beweis stark modifiziert, Original auf der Homepage}
	\[
...
\begin{definition}[{name=[gerichteter Kolimes]}]
	...
	\]
	Für $\lambda_0 \in \Lambda$ sei $s_{\lambda_0} \colon F(\lambda_0) \to \colim_\Lambda F$ die Komposition von der Inklusion $F(\lambda_0) \hookrightarrow \coprod_{\lambda \in \Lambda}F(\lambda)$ mit der Projektion $\coprod_{\lambda \in \Lambda}F(\lambda) \twoheadrightarrow \colim_\Lambda F$. Durch
	\begin{align}
...
\begin{bemerkung}[{name=[{gerichtete Kolimiten}]}]
	\leavevmode
	\begin{itemize}
		...
		\end{enumerate}
		\item Der Kolimes über gerichtete Mengen ist eine Spezialfall des Kolimes über kleine Kategorien aus den Übungen (Blatt 3 Aufgabe1). 
...
\begin{lemma}[{name=[Isomorphismus zum Kolimes einer kofinalen Teilmenge]},label=lem:kofinal_gleicher_kolimes]
...
\end{satz}
\begin{beweis}
	Seien $s_\lambda, s'_\lambda, s''_\lambda$ die Strukturabbildungen.\marginnote{Zur Übung könnte man die fehlenden Teile noch zeigen} 
...
\begin{definition}[{name=[singuläre Kohomologie mit kompaktem Träger]}]
	Sei $X$ ein topologischer Raum. Dann definieren wir die\marginnote{$H^*_c$ ist \emph{kein} Funktor von $\TOP$, um einen Funktor zu erhalten muss man sich auf eigentliche Abbildungen beschränken, d.h. stetige Abbildungen, sodass Urbilder kompakter Mengen wieder kompakt sind! Siehe auch Aufgabe 2 von Blatt 7} \bet{singuläre Kohomologie von $X$ mit kompaktem Träger}\index{Kohomologie mit kompaktem Träger} durch
	\[
...
\begin{beispiel}[{name=[{Kohomologie mit kompaktem Träger von kompakten Räumen}]}]
...
\begin{lemma}[{name=[{Kokettenkomplex mit kompaktem Träger als Unterkomplex}]}]
	$C^*_c(X)$ lässt sich mit dem Unterkomplex von $C^*(X)$ identifizieren,\marginnote{dies begründet die Bezeichnung \enquote{mit kompaktem Träger}} der aus allen $\alpha \colon C_*(X) \to R$ besteht, für die es $K \subseteq X$ kompakt gibt mit $\alpha(\sigma)=0$ für $\sigma \colon \abs*{\Delta^n} \to X\setminus K$.
\end{lemma}
\begin{beweis}
...
\begin{definition}[{name=[{Poincaré-Dualitäts-Abbildung}]}]
...
\end{lemma}
\begin{beweis}
	Ist $B \subset \mathbb{R}^n$ ein kompakter Ball, so ist ${\capp\mu_B} \colon H^k(\mathbb{R}^n,\mathbb{R}^n \setminus B,R) \to H_{n-k}(\mathbb{R}^n)$ ein Isomorphismus nach \autoref{Rn_kompakteBaelle}.
...
\begin{lemma}[{name=[{Isomorphismus in Kohomologie von relativen Kokettenkomplexen}]}]
...
\begin{satz}[{name=[{relative Mayer-Vietoris-Folge in Kohomologie}]},label=satz:MV_relativ_koho]
	...
	Dann lässt sich $\alpha$ schreiben als $\alpha= \alpha_U + \alpha_V$ mit $\alpha_U \in C^n(X,U)$ und $\alpha_V \in C^n(X,V)$ und es ist $\partial [\alpha] = \benbrace*{d^n \alpha_U}= \benbrace*{-d^n \alpha_V}$
\end{satz}
\begin{beweis}
...
\begin{satz}[{name=[{Mayer-Vietoris-Folge in Kohomologie mit kompaktem Träger}]}]
...
\begin{erinnerungA}[{name=[{Konstruktion der Mayer-Vietoris-Folge in Homologie}]}]
...
\begin{proposition}[{name=[{Mayer-Vietoris-Leiter mit Dualitätsabbildungen}]}]
	Seien $U,V$ offene Teilmengen einer $R$-orientierten $n$-Mannigfaltigkeit $M$.\marginnote{vgl. \cite[Lemma 3.36]{Hatcher}, bei ihm aber nur Kommutativität bis auf Vorzeichen (?)} 
	Dann bilden die Poincaré-Dualitätsabbildungen zusammen mit der Mayer-Vietorisfolge ein kommutatives Diagramm
	\[
		\hspace{-.68ex}\begin{tikzcd}[column sep=1.67em]
			\ldots \rar & H^k_c(U \cap V) \rar \dar["\PD_{U \cap V}"] & H^k_c(U) \oplus H^k_c(V) \dar["\PD_{U} \oplus \PD_{V}"] \rar & H^k_c(U \cup V) \rar["\partial"] \dar["\PD_{U \cup V}"]& H^{k+1}_c(U \cap V) \dar["\PD_{U \cap V}"] \rar & \ldots  \\
			\ldots \rar & H_{n-k}(U \cap V) \rar & H_{n-k}(U) \oplus H_{n-k}(V) \rar & H_{n-k}(U \cup V) \rar["\partial"] & H_{n-k-1}(U \cap V) \rar & \ldots  
		\end{tikzcd}
	\]
\end{proposition}
\begin{beweis}
	Ohne Einschränkungen sei $M=U \cup V$. 
	Die Kommutativität der ersten beiden Quadrate folgt aus der Natürlichkeit des $\capp$-Produktes. \marginnote{siehe auch \autoref{bem:herleitung_PD}}
	...
	Dann ist $\delta [\alpha]= \benbrace*{d^k \alpha_K} = \benbrace*{d^k \alpha_L}$.\marginnote{$U \cup V = U \setminus L  \cup V \setminus K \cup U \cap V$}
...
\begin{satz}[{name=[Poincarés Dualitätssatz]}]
...
\begin{satz}[name={Kragensatz},label=kragensatz]
	Sei $M$ eine kompakte Mannigfaltigkeit mit Rand $\partial M$.\marginnote{Eine solche Umgebung heißt \Index{Kragenumgebung}; die Behauptung lässt sich mit einer Zerlegung der Eins auch für parakompakte Mannigfaltigkeiten beweisen} 
...
\begin{beweis}
	...
	Lemma 16.11 aus Topologie \RM{1}. liefert dann die gewünschte Orientierungsklasse $\mu \in H_n(M, \partial M)$.
\end{beweis}
...
\begin{beweis}
	...
	\[
		\begin{tikzcd}[sep=1.8em]
			\underset{=H_n(N)=0}{H_n(N \times [0,1])} \rar & \underset{\neq 0}{H_n \enbrace[\big]{N \times [0,1], N \times \set*{0,1}}} \rar & \underset{= H_{n-1}(N) \oplus H_{n-1}(N)}{H_{n-1} \enbrace[\big]{N \times \set*{0,1}}} \rar & H_{n-1} \enbrace*{N \times [0,1]}
		\end{tikzcd}
	\]
...
\begin{satz}[{name={Poincaré-Dualität für Mannigfaltigkeiten mit Rand}},label=satz:poincare_rand]
	Sei $M$ eine kompakte $R$"=orientierbare $n$-Mannigfaltigkeit, deren Rand $\partial M$ die Vereinigung von zwei $(n-1)$-Mannigfaltigkeiten $\partial_0 M$ und $\partial_1 M$ entlang ihres gemeinsamen Randes\marginnote{$\partial_0 M$, $\partial_1 M$ und $\partial_0 M \cap \partial_1 M$ dürfen leer sein!} 
	\[
...
\begin{definition}[{name=[Schnittform]}]
	...
	Die Abbildung
	\mapdef{\sprod*{\enspace}{\enspace}_R \colon H^k(M;R) \otimes H^{n-k}(M;R)}{R}{\xi \otimes \eta}{(\xi \cupp \eta)(\mu_M)}{}
...
\begin{bemerkung}[{name=[Begründung des Namens \enquote{Schnittform}]}]
	...
	Seien für $i=1,2$ $N_i$ geschlossene $K$-orientierte\marginnote{$K$ Körper} $k_i$-Mannigfaltigkeiten mit Fundamentalklassen $\mu_i \in H_{k_i}(N_i;K)$.
	Seien $f_i \colon N_i \to M$ stetige Abbildungen in eine geschlossene $K$-orientierte $(n=k_0+k_1)$-Mannigfaltigkeit $M$.
	Seien weiter $\xi_i \in H^{n-{k_i}}(M)$ die zu $(f_i)_*(\mu_i)$ Poincaré-dualen Kohomologieklassen. Dann \enquote{gilt}
	\[
...
\end{satz}
\begin{beweis}
	Wir haben einen Isomorphismen\marginnote{universelles Koeffizienten"=Theorem und Poincaré"=Dualtität}
	\begin{align}
	...
	\[
		\eta \longmapsto \xi \enbrace*{\eta \capp \mu_M} \StackText{\ref{bem_augmentation_cap}}{=} \epsilon \enbrace[\big]{\xi \capp (\eta \capp \mu_M)} \StackText{\ref{lem:cup_n_cap}}{=} \epsilon \enbrace[\big]{(\xi \cupp \eta) \capp \mu_M} \StackText{\ref{bem_augmentation_cap}}{=} (\xi \cupp \eta)(\mu_M) = \sprod*{\xi}{\eta}_K
	\]
...
\begin{lemma}[{name=[{Zerlegung eines Vektorraums mittels Bilinearform}]}]
...
\begin{lemma}[label=sig_null,{name=[{Äquivalenz dazu, dass die Signatur 0 ist}]}]
...
\begin{beispiel}[{name=[{Signatur des komplex projektiven Raumes}]}]
	Sei $\alpha \in H^2(\mathbb{C}P^{2k})$ ein Erzeuger.\marginnote{vgl. \autoref{satz:kohomo_ring_projektiv} (ii)}
	Wir wählen die Orientierung $\mu \in H_{4k} \enbrace{\mathbb{C}P^{2k}}$ für die $\alpha^{2k} (\mu)=1$.
	Mit dieser Orientierung ist $\sig \enbrace{\mathbb{C}P^{2k}}=1$.
\end{beispiel}

\begin{satz}[{name=[{Rand einer 4k+1 Mannigfaltigkeit hat Signatur 0}]},label=satz:sig_rand_4k+1_mfkt]
	Sei $W$ eine kompakte orientierte Mannigfaltigkeit mit Rand.
	Falls $\dim W = 4k+1$ ist, so gilt $\sig(\partial W)=0$.
\end{satz}
\begin{beweis}
	Sei $L :=  \im \enbrace[\big]{i^* \colon H^{2k}(W;\mathbb{R}) \to H^{2k}(\partial W;\mathbb{R})}$. 
	Wir überprüfen die Bedingungen aus \autoref{sig_null}:
	\begin{enumerate}[a)]
		...
		\]
		aufgrund der Exaktheit der Paarfolge, da $\mu_{\partial W}=\partial (\mu_{W,\partial W})$, wobei $\mu_{W,\partial W} \in H_{4k+1}(W,\partial W)$ die Orientierungsklasse von $W$ relativ zu $\partial W$ ist (siehe \cref{sub:fundamentalklasse_rand}).
		...
		Es folgt $\dim \im i^* = \dim \ker i_*$.
		Da wir Koeffizienten in $\mathbb{R}$ betrachten, gilt $H^{2k}(W;\mathbb{R}) \cong \Hom \enbrace*{H_{2k}(W),\mathbb{R}}$ und wir können $i^*$ mit $(i_*)^*$ identifizieren.
		Mit \cref{sub:dimensionsargument_rand_mfkt} folgt damit $\dim \im i_* = \dim \im i^*$.
		Es gilt nun 
		\[
			\dim H^{2k}(\partial W) = \dim \ker i_* + \dim \im i_* = \dim \im i^* + \dim \im i^* = 2 \cdot \dim \im i^*
		\]
		also folgt $\dim \im i^* = \frac{1}{2} \dim H^{2k}(\partial W)$.
	\end{enumerate}
...
\begin{korollarB}[{name=[{Für gerade n ist der komplex projektive Raum nicht Rand einer Mannigfaltigkeit}]}]
	$\mathbb{C}P^{2k}$ ist nicht der Rand einer orientierten kompakten Mannigfaltigkeit.
\end{korollarB}

\begin{ausblick}[{name=[{Klassifizierung von geschlossenen 4-Mannigfaltigkeiten}]}]
...
\begin{beweis}
	\begin{enumerate}[a),itemsep=0pt]
		...
		\item Folgt aus der folgenden \autoref{prop:113} und \autoref{satz:5:kettenhomotopie}.\qedhere
	\end{enumerate}
\end{beweis}

\begin{proposition}[label=prop:113,{name=[{Isomorphismus zwischen Kettenhomotopieklassen und Abbildungen in 0-ter Homologie}]}]
...
\begin{beweis}
	...
	\[
		\begin{tikzcd}[row sep=2.3em, column sep=2.7em]
			H_0(P_*) \dar["\varphi"] & P_0 \lar[two heads]  \dar["f_0"] & \ldots \lar["d_1"] & P_n \lar["d_n"']\dar["f_n"] & P_{n+1} \lar["d_{n+1}"'] \dar[dashed,"f_{n+1}"] \\
	...
	\[
		...
		\end{tikzcd}
	\]
	Wir erhalten\marginnote{dass die Abbildung in $\ker d_n'$ liegt, lässt sich leicht nachrechnen}
	\[
...
\begin{satz}[{name=[projektive Dimension von Hauptidealringen]},label=satz:HIR_proj_dim]
...
\begin{beweis}
	...
	Ist $R$ ein Hauptidealring,\marginnote{die Ringe mit dieser Eigenschaft sind genau die HIR, siehe Aufgabe 4, Blatt 2} so ist $P_1 := \ker \epsilon$ als Untermodul eines freien Moduls wieder frei und wir erhalten wir eine projektive Auflösung
	\(
...
\begin{lemma}[{name=[{projektive Auflösungen von kurzen exakten Folgen}]},label=lem:proj_aufl_kes_ketten]
	Sei 
	\[
		\begin{tikzcd}
...
\begin{frage}[{name=[{Isomorphismen von Kohomologiemoduln}]}]
	...
	\end{itemize}
	Genauso für Kettenkomplexe: Sei $C_*$ ein $\mathbb{Z}$-Kettenkomplex und $C_*'$ ein $R$-Kettenkomplex. Gilt dann
	\begin{itemize}
...
\begin{definition}[{name=[{lineare und exakte Funktoren}]}]
	Seien $R$ und $S$ Ringe.
	Ein Funktor $F\colon \Mod{R} \to \Mod{S}$ heißt \bet{$\mathbb{Z}$-linear}\index{Funktor!$\mathbb{Z}$-linearer} oder \bet{additiv}\index{Funktor!additiver},\marginnote{insb. wird die Nullabbildung auf die Nullabbildung geschickt} wenn für $f,g \colon M \to M'$ gilt $F(f + g)= F(f) + F(g)$.
...
\begin{bemerkung}[{name=[exakte Funktoren und (Ko-)Homologie]},label=bem:exakte_funktoren_homologie]
...
\begin{lemma}[{name=[{Exaktheit vom Tensorieren und Hom}]},label=lem:exakt_tens_hom]
...
\end{lemma}
\begin{beweis}
	\marginnote{Die Injektivität von $f$ wird für diese Aussagen nicht benötigt!}\leavevmode
	\begin{enumerate}[(i)]
...
\begin{lemma}[{name=[{Eindeutigkeit von projektiven Auflösungen nach Tensorieren und Hom}]},label=lem:eind_tensor_hom_proj_aufl]
...
\begin{beweis}
	...
	Beide Funktoren erhalten Kettenhomotopieäquivalenzen (bzw. wandeln diese in Kokettenhomotopieäquivalenzen um) und somit folgt der erste Teil der Aussage.
...
\begin{definition}[{name=[Tor und Ext]}]
	Wir setzen in der Situation von obigem \autoref{lem:eind_tensor_hom_proj_aufl}\marginnote{$\Tor$ und $\Ext$ messen, wie exakt die Funktoren $V \otimes_R - $ bzw. $\Hom(-,W)$ sind}
	\[
		\Tor_n^R(V,M) := H_n(V \otimes_R P_*) \qquad \text{ und } \qquad \Ext_R^n(M,W) := H^n \enbrace[\big]{\Hom_R(P_*,W)} 
	\]
\end{definition}
...
\begin{bemerkung}[label=bem:tor_ext_koerper_hir,{name=[{Einfache Berechnungen von Tor und Ext}]}]
	...
	\begin{enumerate}[(i),itemsep=0pt]
		...
		\item Ist $K$ ein Körper, so ist $\Tor_n^K(V,M)=0$ und $\Ext_K^n(M,W)=0$ für $n>0$.\marginnote{vgl. \autoref{bsp:koerper_proj_dim}}
		\item Ist $R$ ein Hauptidealring, so ist $\Tor_n^R(V,M)=0$ und $\Ext_R^n(M,W)=0$ für $n>1$.\marginnote{vgl. \autoref{satz:HIR_proj_dim}}
	\end{enumerate}
	$\Tor^R_n$ ist ein kovarianter Funktor in $V$ und $M$.
	$\Ext_R^n$ ist kontravariant in $M$ und kovariant in $W$.
\end{bemerkung}

\begin{satz}[{name=[{lange exakte Sequenzen für Tor und Ext}]}]
	Sei 
	\(
		\begin{tikzcd}[cramped,sep=small]
...
\begin{bemerkung}[{name=[{Verträglichkeit von Tor und Ext mit (Ko)-Produkten}]}]
	Es gibt natürliche Isomorphismen für $\Tor$:\marginnote{Das Tensorprodukt erhält beliebige direkte Summen, Homologie auch}
	\[
...
\begin{bemerkung}[{name=[{Formeln für den Fall eines Hauptidealringes}]}]
...
\begin{bemerkung}[{name=[{Tor: Auflösung für den \enquote{antensorierten} Modul}]}]
...
\begin{bemerkung}[{name=[{kurze exakte mit injektivem Modul an der ersten Stelle}]}]
...
\begin{definition}[{name=[{injektive Auflösung}]}]
	Eine \Index{injektive Auflösung} einer $R$-Moduls $M$ ist eine lange exakte Folge 
	\[
		\begin{tikzcd}
...
\section[Das Tensorprodukt und Hom als Kettenkomplexe]{Das Tensorprodukt und $\Hom$ als Kettenkomplexe} % (fold)
\label{sec:13}

\begin{definition}[{name=[{Tensorprodukt von Kettenkomplexen}]},label=def:tensor_ketten]
	Sei $R$ ein Ring, $C_*$ ein Kettenkomplex von $R$-Rechtsmoduln und $D_*$ ein Kettenkomplex von $R$-Linksmoduln.
	Dann definieren wir den $\mathbb{Z}$-Kettenkomplex $(C \otimes_R D)_*$ durch\index{Tensorprodukt!von Kettenkomplexen}\marginnote{In Aufgabe 2 von Blatt 3 haben wir gezeigt, dass dies tatsächlich ein Kettenkomplex ist} 
	\[
...
\begin{bemerkung}[{name=[{zur Definition des Tensorprodukts von Kettenkomplexen}]}]
	\leavevmode
	\begin{itemize}[itemsep=0pt]
...
\begin{definition}[{name=[{Hom von Kettenkomplex und Kokettenkomplex}]}]
	Sei $C_*$ ein $R$-Kettenkomplex und $\tilde{D}^*$ ein $R$-Kokettenkomplex.\marginnote{für zwei Kettenkomplexe $p+n=q$}
	...
	\]
	und die Randabbildung durch\marginnote{für Kettenkomplexe \enquote{-} statt \enquote{+}; graduierter Kommutator}
	\(
...
\begin{bemerkung}[{name=[{zur Definition von Hom für Kettenkomplexe}]}]
	...
	\begin{itemize}[itemsep=0pt]
		...
		\item $H_0 \enbrace*{\Hom_R(C,D)_*}$ ist der Modul der Kettenhomotopieklassen von Kettenabbildungen $C \to D$\marginnote{$C,D$ Kettenkomplexe}.
	\end{itemize}
\end{bemerkung}

\begin{lemma}[label=lem:projaufl_iso_hom,{name=[{Tensorieren mit projektiver Auflösung anstatt einem Rechtsmodul}]}]
...
\begin{beweis}
	...
	Da wir $\mathbb{N}$-graduierte Kettenkomplexe betrachten, sind die Randabbildungen in den \enquote{Randfällen} trivial und wir erhalten als eindeutige Darstellung von $\partial^{P \otimes C}(v)$:
	\[
	...
	\begin{figure}[bt]
		...
		\end{tikzcd}
		\caption{Diagramm zum Beweis von \cref{lem:projaufl_iso_hom}}\label{fig:tensor:ketten}
		}
	\end{figure}%
	Insbesondere gilt: 
	\begin{equation}
...
\begin{lemma}[{name=[{Hom mit injektiver Auflösung des Zielmoduls}]}]
	...
	\[
		H^*\enbrace[\big]{\Hom_R(C_*,W)} \longrightarrow H^* \enbrace[\big]{\Hom_R(C_*,I^*)}
	\]
\end{lemma}
\begin{beweis}
...
\begin{bemerkung}[{name=[zu \autoref{lem:projaufl_iso_hom}]}]
...
\begin{definition}[{name=[Doppelkomplex und Totalkomplex]}]
	...
	\]
	Der \Index{Totalkomplex} von $\enbrace*{C_{**}, \partial^v,\partial^h}$ ist der Kettenkomplex $T(C_{**})$ mit 
	\[
...
\begin{definition}[{name=[Filtrierung]}]
	Sei $C_*$ ein Kettenkomplex. Eine \bet{Filtrierung}\index{Filtrierung!Kettenkomplex} auf $C_*$ ist eine Folge 
	\[
...
\begin{beispiel}[{name=[Filtrierungen]},label=bsp:filtrierungen_total]
	\leavevmode
	\begin{itemize}
		...
		\item Sei $C_{**}$ ein Doppelkomplex.\marginnote{siehe auch \autoref{prop:spektral_total}} 
...
\begin{definition}[{name=[homologische Spektralfolge]},label=def:homologische_spektralfolge]
	Eine \Index{homologische Spektralfolge} (im ersten Quadranten) von $R$-Moduln besteht aus folgenden Daten:
	\begin{enumerate}[1.]
...
\begin{definition}[{name=[konvergente Spektralfolge]},label=def:konvergente_spektralfolge]
	Wir sagen eine Spektralfolge $\enbrace*{E^r_{p,q},d^r_{p,q}}$ \bet{konvergiert}\index{Spektralfolge!konvergente} gegen den graduierten $R$-Modul $A_*$, falls es eine endliche Filtrierung $F_s A_*$ von $A_*$ gibt mit\marginnote{mit nichttrivialer Erweiterung kann man zeigen, dass \enquote{Grenzwerte} nicht eindeutig sind}
	\[
...
\begin{bemerkung}[{name=[kollabierende Spektralfolge]},label=bem:kollabierende_spektralfolge]
	Eine Spektralfolge \bet{kollabiert}\index{Spektralfolge!kollabierende} bei $E^r$, falls es auf der $r$-Seite $E^r$ genau eine Spalte $E^r_{p_0,*}$ oder eine Zeile $E^r_{*,q_0}$ gibt, sodass alle Moduln $E^r_{p,q}$ außerhalb dieser Spalte bzw. Zeile trivial sind.
	Gilt $E^r_{p,q}\Rightarrow A_{p+q}$, so folgt\marginnote{das gilt nur für $r>0$ bzw. $r>1$ (?)} 
	\[
		A_n \cong E^r_{p_0,n-p_0} \quad \text{ bzw. } \quad A_n \cong E^r_{n-q_0,q_0}
	\]
\end{bemerkung}

\begin{lemma}[{name=[{exakte Folgen mit Moduln der $2$-Seite}]},label=lem:kes_konvergente_spektralfolge]
	Sei $E^r_{p,q}$ eine Spektralfolge, die gegen $A_{p+q}$ konvergiert. 
	\begin{enumerate}[(i),itemsep=0pt]
		\item Angenommen $E^2_{p,q}=0$ für $p\ge 2$.\marginnote{Die $2$-Seite besteht nur aus zwei Spalten}
		...
		\[
			...
			\end{tikzcd}
		\]
		\item Angenommen $E^2_{p,q}=0$ für $q \ge 2$.\marginnote{Die $2$-Seite besteht nur aus zwei Zeilen} 
...
\begin{beweis}
	\begin{enumerate}[(i)]
		...
		\]
		% \[
		% 	\frac{F_n A_n}{F_{n-1 A_n}} \cong E^\infty_{n,0} \cong E^2_{n,0} \quad \text{ und } \quad \frac{F_{n-1} A_n}{F_{n-2} A_n} \cong E^\infty_{n-1,1} \cong E^2_{n-1,1}
		% \]
		...
		\[
			...
			\end{tikzcd}
		\]
		\item Wir beweisen nun die zweite Aussage:\marginnote{dies war Übungsaufgabe 3 von Blatt 11.}
...
\begin{satz}[name={Vergleichssatz}]
	...
	\[
		\begin{tikzcd}[row sep=1.5em, column sep=2.8em]
			\displaystyle\frac{F_p A_{p+q}}{F_{p-1} A_p+q} \rar["h"] \dar[phantom,"\cong", sloped] & \displaystyle\frac{F'_p A'_{p+q}}{F'_{p-1} A'_{p+q}} \dar[phantom,"\cong", sloped] \\
...
\begin{definition}[{name=[kohomologische Spektralfolge]}]
	Eine \Index{kohomologische Spektralfolge} (im ersten Quadranten) von $R$-Moduln besteht aus folgenden Daten:
	\begin{enumerate}[1.]
...
\begin{lemma}[{name=[{Konvergenz der Spektralfolge}]}]
	Für $r>p,q+1$ ist 
	\[
		E^r_{p,q} \cong E^{r+1}_{p,q} \cong \ldots \cong \frac{F_p H_{p+q}(C_*)}{F_{p-1} H_{p+q}(C_*)}
	\]
	Also gilt $E^r_{p,q} \Rightarrow H_{p+q}$ wie in \autoref{satz:spektralfolge_filtrierung} behauptet.\marginnote{vgl. \autoref{def:konvergente_spektralfolge}}
\end{lemma}
\begin{beweis}
...
\begin{lemma}[{name=[{$r+1$-Seite ist Homologie der $r$-Seite}]}]
	Es gilt
	\[
		E^{r+1}_{p,q} \cong \frac{\ker d^r_{p,q} \colon E^r_{p,q} \to E^r_{p-r,q+r-1}}{\im d^r_{p+r,q-r+1} \colon E^r_{p+r,q-r+1} \to E^r_{p,q}} 
	\]
...
\begin{proposition}[{name=[{Spektralfolgen aus einem Totalkomplex}]},label=prop:spektral_total]
	...
	Seien nun $F^v_s T(C_{**}) := T \enbrace*{C_{*\le s,*}}$ und $F^h_s T(C_{**}) := T \enbrace*{C_{*,*\le s}}$ die \bet{vertikale und die horizontale Filtrierung}\index{Filtrierung!}\marginnote{vgl. \autoref{bsp:filtrierungen_total} und \autoref{bem:konst_seiten_doppelkomplex}} des Totalkomplexes von $C_{**}$.
...
\end{proposition}
\begin{beweis}
	Nach \autoref{satz:spektralfolge_filtrierung} haben wir die folgenden Isomorphismen\marginnote{die horizontale Filtrierung unterscheidet sich quasi durch eine Spiegelung an der Diagonalen von der vertikalen Filtrierung}
	\[
...
\begin{korollar}[{name=[{Homologie des Totalkomplexes bei exakten Spalten/Zeilen}]}]
...
\begin{proposition}[{name=[{Spektralfolgen zum Tensorprodukt von Kettenkomplexen}]},label=prop:spektralfolgen_konv_homlo_tensor]
	Sei $R$ ein Ring, $C_*$ ein Kettenkomplex von $R$-Rechtsmoduln und $D_*$ ein Kettenkomplex von $R$-Linksmoduln.
	\begin{enumerate}[a)]
...
\begin{beweis}
	...
	\begin{enumerate}[a)]
		...
		\[
			E^2_{p,q} \cong H_p \enbrace*{C_* \otimes_R H_q(D_*), \partial^C \otimes \id} \cong H_p \enbrace[\big]{C_* \otimes_R H_q(D_*)}
		\]
...
\begin{satz}[label=satz:homol_tensor,{name=[{Homologie des Tensorproduktes von Kettenkomplexen}]{Künneth}}]
...
\begin{beweis}
	...
	\begin{figure}[hbt]
		\centering{
			\[
				\begin{tikzcd}[row sep=large]
					C_{p} \otimes D_{q+1} \dar["\id \otimes \partial^D",hook] & C_{p+1} \otimes D_{q+1} \dar["\id \otimes \partial^D",hook] \\
					C_p \otimes D_q & C_{p+1} \otimes D_q
				\end{tikzcd}
				\hspace{2.5cm} 
				\begin{tikzcd}[row sep=large,column sep=3em]
					0 & 0 \\
					C_p \otimes H_q(D_*) & C_{p+1} \otimes H_q(D_*) \lar["\partial^C \otimes \id"',"=: d^1"]
				\end{tikzcd}
			\]
			\caption{Die $0$-Seite $E^0_{p,q}$ (links) und die $1$-Seite $E^1_{p,q}$ (rechts) der Spektralsequenz $E^r_{pq}$}\label{fig:homol_tensor:1}
		}
	\end{figure}
	In \cref{fig:homol_tensor:1} sind Ausschnitte der $0$- sowie der $1$-Seite dargestellt.
	Nun ist $\begin{tikzcd}[cramped,sep=small] H_p(C_*) & C_p \lar[two heads] & C_{p+1} \lar[{Hooks[left]}-{Computer Modern Rightarrow}] \end{tikzcd}$ eine freie und damit auch projektive Auflösung von $H_p(C_*)$, also gilt für die einzigen beiden Moduln der $2$-Seite\marginnote{Homomorphiesatz auf der tensorierten exakten Folge}
	\begin{align}
	...
	% 	H_{p+q+1}(C_* \otimes D_*) &\cong \Tor_1^R \enbrace[\big]{H_p(C_*) \otimes H_q(D_*)} \\
	% 	H_n(C_* \otimes D_*)&= 0 \qquad \text{ für } n \notin \set*{p+q,p+q+1} \qedhere
	% \end{align}
\end{beweis}

\begin{bemerkung}[{name=[{zur Natürlichkeit des Isomorphismus in \autoref{satz:homol_tensor}}]},label=bem:exakte_folge_kuenneth]
	...
	\[
		\begin{tikzcd}[column sep=2.0em]
			0 \rar &\displaystyle\smashoperator[r]{\bigoplus_{p+q=n}}\enspace H_p (C_*) \otimes H_q (D_*) \rar["\alpha"] & H_n(C_* \otimes D_*) \rar &
			\displaystyle\smashoperator[r]{\bigoplus_{p+q=n-1}}\enspace \Tor_1^R \enbrace[\big]{H_p(C_*), H_q(D_*)} \rar & 0
		\end{tikzcd} 
	\]
	...
	Diese Folge haben in Aufgabe 3 von Blatt 12 konstruiert, siehe \cref{sub:aufg3_blatt12}.
\end{bemerkung}
...
\begin{satz}[{name={Eilenberg-Zilber}},label=eilenbergzilber]
	...
	\]
	eine Kettenhomotopieäquivalenz, die \Index{Alexander-Whitney-Abbildung}\marginnote{In Aufgabe 2 von Blatt 3 haben wir gezeigt, dass dies tatsächlich eine Kettenabbildung ist} 
	\[
		\AW_* \colon C^\sing_*(X \times Y;R) \longrightarrow C_*^\sing(X;R) \otimes_R C_*^\sing(Y;R)
	\]
\end{satz}

\begin{korollarB}[{name=[{Künneth-Formel für die Homologie eines Produktraums}]}]
	Sei $R$ ein Hauptidealring, $X,Y$ topologische Räume.
	Dann gilt
	\[
		H_n(X \times Y;R) \cong \smashoperator[r]{\bigoplus_{p+q=n}}\enspace H_p(X;R) \otimes_R H_q(Y;R) \oplus \smashoperator[r]{\bigoplus_{p+q=n-1}}\enspace \Tor_1^R \enbrace[\big]{H_p(X;R),H_q(Y;R)}
	\]
\end{korollarB}

\begin{proposition}[{name={Existenz}},label=prop:existenz_eilenberg_zilber]
...
\end{proposition}
\begin{beweis}
	Wir benutzen die \emph{Methode der azyklischen Modelle} (siehe \cref{sub:azyklische_modelle}) und konstruieren 
	\[
...
\begin{beweis}[{name={von Eilenberg-Zilber, \ref{eilenbergzilber}}}]
	Sei $m$ die natürliche Kettenabbildung aus \autoref{prop:existenz_eilenberg_zilber}.
...
\begin{satz}[{name=[{Spektralfolge mit Tor auf der 2-Seite}]},label=satz:spektralfolge_tor_homol_tensor]
...
\begin{beweis}
	...
	\]
	Indem man die durch $\epsilon \otimes \id$ induzierte Abbildung auf Spektralfolgen betrachtet,\marginnote{dass $\epsilon \otimes \id$ den Isomorphismus induziert, brauchen wir für die Natürlichkeit} sieht man, dass $\epsilon \otimes \id$ den Isomorphismus induziert.
...
\begin{satz}[label=satz:hir_tensorprod_homologie,name={Universelles Koeffizienten-Theorem für Homologie von Kettenkomplexen}]
	Sei $R$ ein Hauptidealring, $V$ ein $R$-Modul und $C_*$ ein $R$-Kettenkomplex von freien\marginnote{$\Leftrightarrow$ projektiv, da HIR} $R$-Moduln.
...
\begin{satz}[{name=[{Spektralfolge mit Ext auf der 2-Seite}]},label=satz:spektralfolge_ext]
	Sei $R$ ein Ring, $C_*$ Kettenkomplex von projektiven $R$-Moduln und $W$ ein $R$-Modul.\marginnote{vgl. \autoref{satz:spektralfolge_tor_homol_tensor}}
	Dann gibt es eine kohomologische Spektralfolge 
	\[
...
\begin{beweis}
	...
	\[
		^v E^{p,q}_2 = \begin{cases}
			H^p \enbrace[\big]{\Hom(C_*,W)} &\text{ falls }q=0\\
			0 &\text{ sonst}
		\end{cases}
	\]
	...
	Es folgt
	\[
		H^n \enbrace[\big]{\Hom(C_*,I^*)} \StackText{\ref{bem:kollabierende_spektralfolge}}{\cong} {^v E_2^{n,0}} \cong H^n \enbrace[\big]{\Hom(C_*,W)}
	\]
...
\begin{satz}[label=satz:ext_kettenkompl_kef,name={Universelles Koeffizienten-Theorem für Kohomologie von Kettenkomplexen}]
	Sei $R$ ein Hauptidealring, $C_*$ ein Kettenkomplex von projektiven $R$-Moduln und $W$ ein $R$-Modul.\marginnote{vgl. \autoref{satz:hir_tensorprod_homologie}}
	Dann gibt es eine kurze exakte Folge
	\[
		\begin{tikzcd}
...
\begin{beweis}
	...
	\]
	Den Isomorphismus erhält man am elegantesten aus der \href{https://en.wikipedia.org/wiki/Tensor-hom_adjunction}{Tensor-Hom-Adjunktion}\footnote{\url{https://en.wikipedia.org/wiki/Tensor-hom_adjunction}}, die besagt, dass folgender natürlicher Isomorphismus existiert:\marginnote{siehe auch Aufgabe 1 von Blatt 4}
	\[
...
\section[Symmetrie von Tor und Ext]{Symmetrie von $\Tor$ und $\Ext$} % (fold)
\label{sec:18}
...
\begin{beweis}
	...
	\[
		...
		\end{cases}
	\]
	Die natürliche Spektralfolge kollabiert bei $E^2$ und es gilt $H_n \enbrace*{P_* \otimes_R Q_*} \cong {^v E^2_{n,0}} \cong H_n (P_* \otimes W)$.
	...
	Dies haben wir auch schon in \autoref{satz:spektralfolge_tor_homol_tensor} gezeigt.
\end{beweis}
...
\begin{bemerkung}[{name=[{Symmetrie von Tor}]}]
	...
	\begin{enumerate}[(i)]
		...
		\[
			\Tor_*^R(V,W) = H_*\enbrace*{V \otimes_R Q_*} \cong H_* \enbrace*{Q_* \otimes_R V} \cong \Tor_*^R(W,V)
		\]
	\end{enumerate}
\end{bemerkung}

\begin{satz}[{name=[{Kohomologie von Hom über projektive oder injektive Auflösung}]},label=satz:injektive_aufl_ext]
	Sei $R$ ein Ring.
...
\begin{satz}[{name=[{Äquivalenz von Orientierbarkeit und Existenz eines Orientierungsatlanten}]},label=satz:orientierbar_atlas]
...
\begin{bemerkung}[{name=[{zu \autoref{satz:orientierbar_atlas}}]}]
	\leavevmode
	\begin{enumerate}[i)]
...
\begin{lemma}[{name=[{Orientierungserhaltung von Diffeomorphismen}]}]
...
\begin{korollarB}[{name=[{Orientierbarkeit im homologischen Sinne ist das gleiche wie im differentialtopologischen Sinne}]}]
