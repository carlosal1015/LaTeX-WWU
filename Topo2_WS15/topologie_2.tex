...
\begin{beispiel}[{name=[Filtrierungen]}]
	...
	\begin{itemize}
		...
		Dann erhalten wir zwei Filtrierungen $F^v$ und $F^h$ auf $T(C_{**})$ mit\todo{prüfen, ob Notation so geblieben ist} 
		\[
...
\begin{bemerkung}[{name=[{Konstruktion weiterer Komplexe aus einem Doppelkomplex}]}]
...
\begin{bemerkung}[{name=[{Morphismen von Spektralfolgen und stabiler Wert}]},label=bem:mor_spektralfolge]
	...
	\begin{itemize}
		...
		\item Für $r > \max\set{p,q+1}$ sind $d^r_{p,q}$ und $d^r_{p+r,q-r+1}$ trivial und daher gilt 
		\[
...
\begin{satz}[{name=[{Spektralfolge zu einem Kettenkomplex mit Filtrierung}]}]
...
\end{satz}
\todo[inline]{kommt da wirklich eine 2 hin?}

Wir erhalten folgende Kette von $R$-Moduln zwischen $F_{p-1} C_n \subseteq F_p C_n$:
\begin{align}
...
\begin{lemma}[{name=[{Seiten bleiben stabil für $r >p,q+1$}]}]
	Für $r>p,q+1$ ist 
	\[
...
\[
	\hspace{-1ex}\begin{tikzcd}[row sep=7pt,column sep=5pt]
		E^r_{p,q} \rar["d^r_{p,q}"] \dar[equal]& E^r_{p-r,q+r-1} \dar[equal] \\
...
\begin{beweis}
	...
	Da $\tilde{a} \in d^{-1} \enbrace*{F_{p-r-1} C_{p+q-1}}\cap F_p C_{p+q} \subseteq d^{-1} (F_{p-r-1} C_{p+q-1})$ ist $d\tilde{a} \in F_{p-r-1} C_{p+q-1}$.\todo{Revision 15}
\end{beweis}
