...
\begin{beweis}
	...
	\]
	immer surjektiv ist nach \autoref{eig_kohomo_to_hom_homo} und in diesem Fall injektiv nach \cref{sub:paarung_isomorphismus_frei}\todo{leider gilt auch das nur für $\mathbb{Z}$ und dafür war es sowieso klar … es soll möglich sein, den allgemeinen Fall auf $\mathbb{Z}$ zu reduzieren mit dem universellen Koeffizienten-Theorem}, gibt es genau ein $\xi \in H^n \enbrace*{\mathbb{R}^n,\mathbb{R}^n \setminus B;R}$ mit $\xi(\mu_B)=1$. 
...
\begin{beweis}[{name=[{\cite[Th.~3.43]{Hatcher}}]}]
	...
	Für die Kommutativität im zweiten Quadrat benötig man die Tatsache, dass die Randabbildung $H_n(M,\partial M) \to H_{n-1}(\partial M)$ Fundamentalklassen von $M$ auf Fundamentalklassen von $\partial M$ abbildet.\todo{In den Anhang?}
...
\begin{bemerkung}[{name=[{über reelle Koeffizienten}]}]
	\leavevmode
	\begin{itemize}
		\item Es ist $H_n(M;\mathbb{R}) \cong H_n(M;\mathbb{Z})\otimes \mathbb{R}$ nach dem \hyperref[satz:univ_koeff_homo_space]{universellen Koeffzienten-Theorem für Homologie}.\todo{noch einmal nachrechnen}
...
\begin{beweis}
	...
	\begin{enumerate}[a)]
		...
		\]
		aufgrund der Exaktheit der Paarfolge, da $\mu_{\partial W}=\partial (\mu_{W,\partial W})$, wobei $\mu_{W,\partial W} \in H^{4k+1}(W,\partial W)$ die Orientierungsklasse von $W$ relativ zu $\partial W$ ist.\todo{Referenz zur Übungsaufgabe}
		...
		Da $\dim H^{2k}(\partial W) = \dim \ker i^* + \dim \im i^*$, folgt $\dim \im i^* = \frac{1}{2} \dim H^{2k}(\partial W)$.\todo{hier stimmt das noch nicht ganz}
	\end{enumerate}
...
\begin{bemerkung}[{name=[zu \autoref{lem:projaufl_iso_hom}]}]
	...
	Dies stellt einen Zusammenhang zwischen $H_*(V \otimes C_*)$ und $\Tor_*^R \enbrace*{V, H_*(C_*)}$ her.\todo{nochmal durchgehen, wenn die nächsten Kapitel geschafft sind}
\end{bemerkung}
...
\begin{definition}[{name=[konvergente Spektralfolge]},label=def:konvergente_spektralfolge]
	Wir sagen eine Spektralfolge $\enbrace*{E^r_{p,q},d^r_{p,q}}$ \bet{konvergiert}\index{Spektralfolge!konvergente} gegen den graduierten $R$-Modul $A_*$, falls es eine endliche Filtrierung $F_s A_*$ von $A_*$ gibt mit
	\[
...
\begin{bemerkung}[{name=[{zur Natürlichkeit des Isomorphismus in \autoref{satz:homol_tensor}}]}]
	Der Isomorphismus in \autoref{satz:homol_tensor} ist nicht natürlich in $C_*$ und $D_*$.\todo{genauer?}
...
\begin{satz}[{name=[{Kohomologie von Hom über projektive oder injektive Auflösung}]}]
	Sei $R$ ein Ring.
