...
\begin{satz}[{name=[{Rand einer 4k+1 Mannigfaltigkeit hat Signatur 0}]}]
...
\begin{beweis}
	...
	\begin{enumerate}[a)]
		...
		\]
		wegen Exaktheit der Paarfolge, da $\mu_{\partial W}=\partial (\mu_{W,\partial W})$, wobei $\mu_{W,\partial W} \in H^{4k+1}(W,\partial W)$ die Orientierungsklasse von $W$ relativ zu $\partial W$ ist.\todo{Referenz zur Übungsaufgabe}
		...
		\[
			\begin{tikzcd}[sep=large]
				...
				\ldots \rar & H_{2k+1}(W,\partial W) \rar["\partial"] & H_{2k}(\partial W) \rar["i_*"] & H_{2k}(W) \rar & \ldots 
			\end{tikzcd}
		\]
		Die Kommutativität benutzt wieder $\partial (\mu_{W,\partial W})=\mu_{\partial W}$.
		Es gilt\todo{ich habe keine Ahnung, was der dritte Ausdruck sein soll}
		\[
...
\begin{frage}[{name=[{Isomorphismen von Kohomologiemoduln}]}]
	...
	\begin{itemize}
		\item $H_*(X;V) \cong H_*(X) \otimes V$ ?\todo{Definition von Homologie mit Koeffizienten in abelscher Gruppe}
...
\begin{bemerkung}[{name=[exakte Funktoren und (Ko-)Homologie]}]
	...
	Ist $F \colon \Mod{R} \to \Mod{S}$ exakt, so gilt\todo{Revision 12}
	\[
		\begin{array}{ll}
