...
\begin{bemerkung}[{name=[{über reelle Koeffizienten}]}]
	\leavevmode
	\begin{itemize}
		\item Es ist $H_n(M;\mathbb{R}) \cong H_n(M;\mathbb{Z})\otimes \mathbb{R}$ nach dem \hyperref[satz:univ_koeff_homo]{universellen Koeffzienten-Theorem für Homologie}.
...
\begin{satz}[{name=[Existenz projektiver Auflösungen]},label=satz:ex_eind_proj_aufl]
...
\end{lemma}
\begin{beweis}
	\marginnote{Die Injektivität von $f$ wird für diese Aussagen nicht benötig!}\leavevmode
	\begin{enumerate}[(i)]
...
\begin{bemerkung}[{name=[{zur Definition des Tensorprodukts von Kettenkomplexen}]}]
	...
	\begin{itemize}[itemsep=0pt]
		...
		\item Ist $R$ kommutativ, so ist $C \otimes_R D$ ein $R$-Kettenkomplex.\todo{Revision 13}
	\end{itemize}
\end{bemerkung}

\begin{definition}[{name=[{Hom von Kettenkomplexen}]}]
...
\begin{satz}[{name=[{Universelles Koeffizienten-Theorem für Homologie}]},label=satz:univ_koeff_homo]
