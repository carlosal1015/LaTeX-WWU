...
\begin{definition}[{name=[{lineare und exakte Funktoren}]}]
	Seien $R$ und $S$ Ringe.
	Ein Funktor $F\colon \Mod{R} \to \Mod{S}$ heißt \bet{$\mathbb{Z}$-linear}\index{Funktor!$\mathbb{Z}$-linearer},\marginnote{insb. wird die Nullabbildung auf die Nullabbildung geschickt} wenn für $f,g \colon M \to M'$ gilt $F(f + g)= F(f) + F(g)$.
...
\begin{bemerkung}[{name=[exakte Funktoren und (Ko-)Homologie]},label=bem:exakte_funktoren_homologie]
	...
	Ist $F \colon \Mod{R} \to \Mod{S}$ exakt, so gilt\todo{nochmal detailliert durchgehen, Aufgabe 2 Blatt 8}
	\[
		\begin{array}{ll}
...
\begin{beweis}
	...
	Nun ist $\begin{tikzcd}[cramped,sep=small] H_p(C_*) & C_p \lar[two heads] & C_{p+1} \lar[{Hooks[left]}-{Computer Modern Rightarrow}] \end{tikzcd}$ eine freie und damit auch projektive Auflösung von $H_p(C_*)$, also gilt für die einzigen beiden Moduln der $2$-Seite\todo{das zweite \enquote{=} in der ersten Zeile verstehe ich noch nicht}
	\begin{align}
