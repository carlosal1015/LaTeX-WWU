...
\begin{enumerate}
	...
	\item Die lineare PDE der Form 
	\[
		D_{11}u(x) - \sum^{n}_{i,j=1} a_{ij}(x)D_{ij}u(x) + \sum^{n}_{i=1}b_i(x) D_iu(x) + c(x)u(x) - f(x) = 0 \qquad \text{in }\Omega
	\]
...
\begin{align*}
	...
	&\Rightarrow v'(r) = \eta r^{1-n},
\end{align*}
für eine beliebige Konstante $\eta \in \mathbb{R}$. Es gilt
\begin{equation}
	v'(r) = \begin{cases}
		\frac{\eta}{r}, &\text{ falls } n = 2,\\
		\eta r^{1-n}, &\text{ falls } n \geq 3		
	\end{cases}
\end{equation}
und daraus folgt
\begin{equation}
