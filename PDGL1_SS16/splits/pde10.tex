...
\begin{beweis}
	...
	\begin{align*}
		...
		& \qquad \qquad - \frac{1}{2}\int_{\partial B_{t_0-t}(x_0)}^{}((u_t(t,x))^2 + \abs{ \nabla u(t,x)}^2) \,\mathrm{d}S(x) \\
...
\begin{beispiel}
	\begin{enumerate}[1.]
		...
		\end{enumerate}
		Auch hier werden nur die Gleichungen für $\gamma$ und $v$ benötigt.
	\end{enumerate}
\end{beispiel}
...
\]
\[
	z_i(0) = \partial_{x_i} g(x_0), \qquad i=1,\dots,n-1
\]
...
\begin{definition*}
	Ein Vektor $(x_0,v_0,z_0) \in \mathbb{R}^{2n+1}$ heißt zulässig für das Anfangswertproblem (2), falls \[
